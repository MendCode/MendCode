...
const APP_IDS: Record<string, string> = {
  dev: "ai.mendcode.desktop.dev",
  beta: "ai.mendcode.desktop.beta",
  prod: "ai.mendcode.desktop",
}
const TEST_ONBOARDING = process.env.OPENCODE_TEST_ONBOARDING === "1"
const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.mendcode.desktop.dev"
const onboardingTestRoot = setupOnboardingTestEnv()
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "MendCode Dev")
...
function setupApp() {
  ...
  void app.whenReady().then(async () => {
    ...
    app.setAsDefaultProtocolClient("mendcode")
    registerRendererProtocol()
    setDockIcon()
    setupAutoUpdater()
    await initialize()
...
async function initialize() {
  ...
  const needsJsonStartupMigration = !existsSync(JsonMigration.jsonStorageMigrationDonePath())
  const sqliteDone = needsJsonStartupMigration ? defer<void>() : undefined
