...
import { Script } from "@mendcode/script"
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    entrypoints: ["./src/index.ts", parserWorker, workerPath, "./src/mend/codemode/worker.ts", ...(embeddedFileMap ? ["opencode-web-ui.gen.ts"] : [])],
    define: {
      OPENCODE_VERSION: `'${Script.version}'`,
      OPENCODE_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
      OPENCODE_WORKER_PATH: workerPath,
      MENDCODE_CODE_WORKER_PATH: "./src/mend/codemode/worker.ts",
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    ...
    try {
      ...
      if (checkHelp) {
        ...
        if (!helpOutput.trim()) {
          throw new Error("--help produced no output")
        }
      }
...
if (Script.release || releaseAssetsFlag) {
  ...
  await $`sh -c "find . -maxdepth 1 \\( -name '*.zip' -o -name '*.tar.gz' \\) -print | sort | xargs shasum -a 256 > SHA256SUMS"`.cwd(
    "./dist",
  )
}
