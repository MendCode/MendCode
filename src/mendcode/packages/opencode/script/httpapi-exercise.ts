...
void (await import("@mendcode/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "mendcode.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "mendcode.jsonc")).text(),
          )
          ...
          message: (sessionID, input) =>
            Effect.gen(function* () {
              const info: MessageV2.User = {
                ...
                agent: "build",
                model: {
                  providerID: ProviderID.mendcode,
