...
- Author package entrypoint is `@mendcode/plugin/tui`.
...
- Import types from `@mendcode/plugin/tui`.
- `@mendcode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running MendCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.mendcode` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.mendcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.mendcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.mendcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.mendcode` dir or `OPENCODE_CONFIG_DIR`, MendCode installs `@mendcode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@mendcode/plugin/tui`.
...
- Local plugins persist installed themes under the local `.mendcode/themes` area near the plugin config source.
...
- Local smoke plugin: `.mendcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.mendcode/plugins/tui-vim.tsx`
- Local smoke config: `.mendcode/tui.json`
- Local smoke theme: `.mendcode/plugins/smoke-theme.json`
