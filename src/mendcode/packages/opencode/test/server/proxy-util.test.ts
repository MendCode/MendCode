...
import { HEADER_DIRECTORY, HEADER_WORKSPACE } from "../../src/server/shared/header-names"
    ...
    test("strips opencode-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          ...
          [HEADER_DIRECTORY]: "/home/user/project",
          [HEADER_WORKSPACE]: "ws_456",
      ...
      expect(result.get(HEADER_DIRECTORY)).toBeNull()
      expect(result.get(HEADER_WORKSPACE)).toBeNull()
