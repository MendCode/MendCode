...
describe("mend tui presentation renderers", () => {
  ...
  test("uses the active phase instead of a generic continuation label", () => {
    ...
    expect(toolContinuationActivity({ status: "pending", tool: "read" })).toBe("Preparing tool call...")
    expect(toolContinuationActivity({ status: "pending", tool: "bash" })).toBe("Preparing tool call...")
