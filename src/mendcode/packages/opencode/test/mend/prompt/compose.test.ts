...
describe("mend prompt composition", () => {
  ...
  test("full mode advertises TUI markdown and Mermaid rendering", async () => {
    ...
    expect(section?.text).toContain("30 built-in local families")
    expect(section?.text).toContain("flowchart/graph")
  ...
  test("full mode explains marketplace extension boundaries", async () => {
    ...
    expect(section?.text).toContain("mendcode theme validate <file>")
    expect(section?.text).toContain("mendcode theme install <file>")
    expect(section?.text).toContain("mendcode theme select <name>")
    expect(section?.text).toContain(".mendcode/themes/")
    expect(section?.text).toContain("Do not use `kv.json`")
    expect(section?.text).toContain("missing/circular references")
