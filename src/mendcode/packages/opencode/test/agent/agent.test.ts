...
function loadPlanExitAgent(dir: string) {
  return Effect.runPromise(
    provideInstance(dir)(
      Effect.gen(function* () {
        const config = yield* Config.Service
        const agents = yield* Agent.Service
        return yield* resolvePlanExitAgent({ config, agents })
      }),
    ).pipe(Effect.provide(Agent.defaultLayer), Effect.provide(Config.defaultLayer)),
  )
}
...
test("plan agent denies edits except .mendcode/plans/*", async () => {
  await using tmp = await tmpdir()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".mendcode/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("Agent.get resolves legacy sub-prefixed aliases for custom subagents", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        "code-reviewer": {
  ...
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      const direct = await load(tmp.path, (svc) => svc.get("code-reviewer"))
      const prefixed = await load(tmp.path, (svc) => svc.get("sub/code-reviewer"))

      expect(direct?.name).toBe("code-reviewer")
      expect(prefixed?.name).toBe("code-reviewer")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".mendcode", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await WithInstance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".mendcode", "skill", "perm-skill")
...
test("defaultAgent uses custom primary when build and plan are disabled", async () => {
  await using tmp = await tmpdir({
    config: {
      ...
      agent: {
        ...
        custom_primary: {
          mode: "primary",
          description: "Custom primary mode",
        },
      },
    },
  })
  await WithInstance.provide({
    ...
    fn: async () => {
      const agent = await load(tmp.path, (svc) => svc.defaultAgent())
      expect(agent).toBe("custom_primary")
    },
  })
})

test("plan exit agent defaults to build", async () => {
  await using tmp = await tmpdir()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      const agent = await loadPlanExitAgent(tmp.path)
      expect(agent).toBe("build")
    },
  })
})

test("plan exit agent respects configured primary agent", async () => {
  await using tmp = await tmpdir({
    config: {
      plan_exit_agent: "custom_primary",
      agent: {
        custom_primary: {
          mode: "primary",
          description: "Custom implementation mode",
        },
      },
    },
  })
...
test("plan exit agent falls back to build for subagents", async () => {
  await using tmp = await tmpdir({
    config: {
      plan_exit_agent: "explore",
    },
  })
  await WithInstance.provide({
    directory: tmp.path,
