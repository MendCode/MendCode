...
test("shell ended updates the matching shell by callID", () => {
  const state: SessionMessageUpdater.MemoryState = { messages: [] }
  const sessionID = SessionID.make("session")
  const callID = "shell-call"
  ...
  if (state.messages[0]?.type !== "shell") return
  expect(state.messages[0].callID).toBe(callID)
  expect(state.messages[0].output).toBe("hello")
