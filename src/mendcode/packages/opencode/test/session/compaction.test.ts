...
describe("session.compaction.process", () => {
      ...
      fn: async () => {
        ...
        try {
          ...
          if (last?.parts[0]?.type === "text") {
            ...
            expect(last.parts[0].text).toContain("Do not ask for confirmation before continuing required active work")
            expect(last.parts[0].text).not.toContain('If Active Work is "(none)"')
            expect(last.parts[0].text).not.toContain("Continue if you have next steps")
          }
        } finally {
          await rt.dispose()
        }
  ...
  test("compaction prompt separates active work from optional follow-ups", async () => {
    const stub = llm()
    let captured = ""
    stub.push(
      reply("summary", (input) => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        try {
          ...
          expect(captured).toContain("## Current User Intent")
          expect(captured).toContain("Latest explicit user request")
          expect(captured).toContain("Explicitly not requested")
          expect(captured).toContain("## Resume Anchor")
          expect(captured).toContain("Current task in progress")
  ...
  test("manual and automatic compactions use the same anchored summary prompt", async () => {
    async function capturePrompt(auto: boolean) {
      const stub = llm()
      let captured = ""
      stub.push(
        reply("summary", (input) => {
      ...
      return WithInstance.provide({
        ...
        fn: async () => {
          const session = await svc.create({})
          await user(session.id, "finish feature then stop")
          await SessionCompaction.create({
            sessionID: session.id,
            agent: "build",
            model: ref,
    ...
    const manual = await capturePrompt(false)
    const automatic = await capturePrompt(true)

    for (const prompt of [manual, automatic]) {
      expect(prompt).toContain("Create a new anchored summary from the conversation history above")
  ...
  test("manual compaction focus is passed to the summary prompt", async () => {
    const stub = llm()
    let captured = ""
    stub.push(
      reply("summary", (input) => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const session = await svc.create({})
        await user(session.id, "implement dashboard auth")
        await SessionCompaction.create({
          sessionID: session.id,
          agent: "build",
          model: ref,
          auto: false,
          instructions: "focus on unfinished TODOs and subagent outputs",
        })
    ...
    expect(captured).toContain("User compaction focus")
    expect(captured).toContain("focus on unfinished TODOs and subagent outputs")
    expect(captured).toContain("Do not treat it as a new task to execute")
  })

  test("compaction prompt includes persisted TODO state as context", async () => {
    const stub = llm()
    let captured = ""
    stub.push(
      reply("summary", (input) => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const session = await svc.create({})
        await user(session.id, "continue the active work")
        await Effect.runPromise(
          Todo.Service.use((svc) =>
            svc.update({
              sessionID: session.id,
              todos: [
                { content: "Fix auto compaction loop", status: "in_progress", priority: "high" },
                { content: "Run focused compaction tests", status: "pending", priority: "medium" },
              ],
            }),
        ...
        try {
          ...
          expect(captured).toContain("Current TODO List")
          expect(captured).toContain("[in_progress] (high) Fix auto compaction loop")
          expect(captured).toContain("[pending] (medium) Run focused compaction tests")
          expect(captured).toContain("Use TODOs as state evidence, not as automatic instructions")
        } finally {
          await rt.dispose()
        }
      },
  ...
  test("compaction prompt includes subagent task outputs as context", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const session = await svc.create({})
        const msg = await user(session.id, "dale siii haz todo eso porfa")
        ...
        await svc.updatePart({
          ...
          state: {
            status: "completed",
            input: {
              description: "Auth admin implementation",
              subagent_type: "convex-expert",
            },
            output: [
              "task_id: sub-session-1",
              "",
              "<task_result>",
              "Mapped Convex Auth. Next required change is bootstrap super admin before review.",
              "Changed files: convex/auth.ts, convex/lib/auth.ts",
              "</task_result>",
            ].join("\n"),
            title: "Auth admin implementation",
