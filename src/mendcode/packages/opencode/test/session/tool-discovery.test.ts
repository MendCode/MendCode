...
describe("tool discovery", () => {
  test("keeps core tools and makes secondary schemas available after discovery", async () => {
    const tools = {
      read: definition("Read files"),
      notes_find: definition("Search notes and memory"),
      screenshot: definition("Capture the computer screen"),
    }
        ...
        parts: [
          {
            type: "tool",
            tool: "tool_search",
            state: { status: "completed", metadata: { discoveredTools: ["screenshot", "denied_tool"] } },
          },
        ],
      },
  ...
  test("caps search results and omits schemas that cannot fit without truncating them", async () => {
    ...
    const giant = tool({
      inputSchema: jsonSchema({ type: "object", description: "x".repeat(30000) }),
      description: "giant",
    })
