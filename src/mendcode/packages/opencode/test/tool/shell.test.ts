...
describe("tool.shell", () => {
  ...
  each("sets safe pipe-friendly environment hints", async () => {
    const prev = process.env.PYTHONUNBUFFERED
    ...
    try {
      await WithInstance.provide({
        ...
        fn: async () => {
          const bash = await initShell()
          const text = `${bin} -e ${evalarg("console.log(process.env.PYTHONUNBUFFERED)")}`
          const result = await Effect.runPromise(
            bash.execute(
              {
                command: PS.has(sh()) ? `& ${text}` : text,
                description: "Checks shell env hints",
              },
              ctx,
            ),
          )
      ...
      if (prev === undefined) delete process.env.PYTHONUNBUFFERED
      else process.env.PYTHONUNBUFFERED = prev
    }
  })
...
describe("tool.shell abort", () => {
  ...
  test("keeps silent commands honest while running through pipes", async () => {
    await WithInstance.provide({
      directory: projectRoot,
      fn: async () => {
        const bash = await initBash()
        ...
        const result = await Effect.runPromise(
          bash.execute(
            {
              command: `sleep 0.1`,
              description: "Silent command test",
            },
            {
              ...ctx,
              metadata: (input) =>
                Effect.sync(() => {
                  updates.push(String((input.metadata as { output?: string })?.output ?? ""))
                }),
            },
          ),
        )
        expect(result.output).toBe("(no output)")
        expect(updates.every((output) => output === "")).toBe(true)
      },
    })
  })

  test("renders carriage-return progress as pipe line updates", async () => {
    await WithInstance.provide({
      directory: projectRoot,
      fn: async () => {
        const bash = await initBash()
        ...
        const result = await Effect.runPromise(
          bash.execute(
            {
              command: `printf 'progress 1\\r' && sleep 0.1 && printf 'progress 2\\r' && sleep 0.1 && printf 'done\\n'`,
              description: "Carriage return progress test",
            },
            {
              ...ctx,
              metadata: (input) =>
        ...
        expect(result.output).toContain("progress 1")
        expect(result.output).toContain("progress 2")
        expect(updates.some((output) => output.includes("progress 1progress 2"))).toBe(false)
