...
async function markPluginDependenciesReady(dir: string) {
  ...
  await Bun.write(
    ...
    JSON.stringify({ packages: { "": { dependencies: { "@mendcode/plugin": "0.0.0" } } } }),
  )
}
...
test("provider config accepts disabling chunk timeout", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "mendcode.json"),
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      const providers = await list()
      expect(providers[ProviderID.anthropic]).toBeDefined()
      expect(providers[ProviderID.anthropic].options.chunkTimeout).toBe(false)
    },
  })
})
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".mendcode", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
