...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "mendcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "mendcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mendcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mendcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["mendcode.json", "mendcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "mendcode.json"),
        path.join(tmp.path, "mendcode.jsonc"),
        path.join(project, "mendcode.json"),
        path.join(project, "mendcode.jsonc"),
      ])
    })
  })
