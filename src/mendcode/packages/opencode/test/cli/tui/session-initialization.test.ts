...
describe("session route initialization", () => {
  test("does not render an empty-editor preflight frame before the optimistic turn exists", async () => {
    ...
    expect(submit.indexOf("await findSkill")).toBeLessThan(submit.indexOf("setSubmitPreflightActive(true)"))
    expect(submit.indexOf("setSubmitPreflightActive(true)")).toBeLessThan(submit.indexOf("props.onSubmit?.({"))
    expect(submit.indexOf("props.onSubmit?.({")).toBeLessThan(submit.indexOf("insertOptimisticUserTurn({"))
    expect(submit.indexOf("insertOptimisticUserTurn({")).toBeLessThan(submit.indexOf("clearPromptForSubmit()"))
    expect(submit.indexOf("props.onSubmit?.({")).toBeLessThan(submit.indexOf("clearPromptForSubmit()"))
  ...
  test("holds the submitted turn by message identity across relayouts", async () => {
    ...
    expect(source).toContain("if (!child || child.height <= 0 || child.y < scroll.y) return false")
    expect(source).toContain(";[16, 50, 120, 240, 480].forEach((delay) =>")
    expect(source).not.toContain("queueMicrotask(() => reconcileSubmitScrollIntent(token))")
    const reconcileStart = source.indexOf("const reconcileSubmitScrollIntent =")
    const reconcileEnd = source.indexOf("const scheduleSubmitScrollIntent =", reconcileStart)
    const reconcile = source.slice(reconcileStart, reconcileEnd)
    expect(reconcile.indexOf("scroll.scrollBy(delta)")).toBeLessThan(reconcile.indexOf("setFollowSessionOutput(false)"))
    const activateStart = source.indexOf("const activateSubmitScrollIntent =")
    const activateEnd = source.indexOf("const syncScrollFollowMode =", activateStart)
    expect(source.slice(activateStart, activateEnd)).not.toContain("setFollowSessionOutput(false)")
