...
async function writeManagedSettings(settings: object, filename = "mendcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "mendcode.json") {
...
test("loads subagent_model without changing main model", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      const config = await load()
      expect(config.model).toBe("test/main")
      expect(config.small_model).toBe("test/small")
      expect(config.subagent_model).toBe("test/subagent")
      expect(config.subagent_variant).toBe("low")
    },
  })
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "mendcode.json"))
...
test("ignores legacy tui keys in runtime config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://mendcode.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "mendcode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "mendcode.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .mendcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mendcode")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".mendcode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .mendcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mendcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads agents from .mendcode/agents/sub using plain names", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentsDir = path.join(dir, ".mendcode", "agents", "sub")
...
Review agent prompt`,
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect(config.agent?.["code-reviewer"]).toMatchObject({
        name: "code-reviewer",
        model: "test/model",
        mode: "subagent",
        prompt: "Review agent prompt",
      })
      expect(config.agent?.["sub/code-reviewer"]).toBeUndefined()
    },
  })
})

test("loads commands from .mendcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mendcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .mendcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mendcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "mendcode.json"),
        JSON.stringify({ $schema: "https://mendcode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .mendcode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".mendcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .mendcode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "mendcode.json"),
        JSON.stringify({
          $schema: "https://mendcode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mendcode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "mendcode.json"),
        JSON.stringify({
          $schema: "https://mendcode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".mendcode")
...
test("local .mendcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mendcode directory config enables it
      const opencodeDir = path.join(dir, ".mendcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "mendcode.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "mendcode.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.mendcode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.mendcode/plugin/demo.ts", "file:///project/.mendcode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.mendcode/plugin/demo.ts"])
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .mendcode/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .mendcode directory with a command
          const opencodeDir = path.join(dir, ".mendcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await WithInstance.provide({
        ...
        fn: async () => {
          ...
          // Project .mendcode should NOT be in directories list
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "MendCode Managed",
          PayloadIdentifier: "ai.mendcode.managed.test",
          PayloadType: "ai.mendcode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://mendcode.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://mendcode.ai/config.json")
})
