...
afterEach(async () => {
  ...
  await fs.rm(path.join(Global.Path.config, "mendcode.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "mendcode.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".mendcode")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "mendcode.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, ".mendcode", "tui.json"),
...
test("migrates tui-specific keys from mendcode.json when tui.json does not exist", async () => {
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "mendcode.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "mendcode.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when mendcode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "mendcode.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "mendcode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "mendcode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "mendcode.json.tui-migration.bak"))).toBe(false)
})

test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "mendcode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "mendcode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "mendcode.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple mendcode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "mendcode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "mendcode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(customDir, "mendcode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENCODE_CONFIG = path.join(customDir, "mendcode.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .mendcode/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".mendcode"), { recursive: true })
      await Bun.write(path.join(dir, ".mendcode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
...
test("silently skips malformed tui.json — load failures degrade to {}", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, ".mendcode", "tui.json"), JSON.stringify({ theme: "fallback" }))
    },
  })

  const config = await getTuiConfig(tmp.path)
  // Project tui.json is malformed → silently skipped (logs a warning)
  // .mendcode/tui.json (lower precedence in this path) still loads
...
test("silently skips non-ENOENT read failures (e.g. tui.json is a directory) — fallback layer still loads", async () => {
  ...
  const config = await getTuiConfig(tmp.path)
  // Did NOT crash; .mendcode/tui.json (lower precedence) still loads.
  expect(config.theme).toBe("fallback")
})
