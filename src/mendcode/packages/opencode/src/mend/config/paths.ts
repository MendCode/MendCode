...
export function mendRuntimeRoot() {
  ...
  for (let i = 0; i < 12; i++) {
    if (
      existsSync(path.join(current, ".mendcode")) &&
      existsSync(path.join(current, "src", "mendcode", "packages", "opencode", "src", "mend", "assets"))
    )
      return current
...
export function mendPaths(root = mendRuntimeRoot()) {
  return {
    ...
    runtimeBackgroundDaemon: path.join(
      root,
      "src",
      "mendcode",
      "packages",
    ...
    runtimePromptEvidenceAssets: path.join(
      root,
      "src",
      "mendcode",
      "packages",
    ...
    promptSourcesMetadata: path.join(
      root,
      "src",
      "mendcode",
      "packages",
    ...
    promptCustom: path.join(root, ".mendcode", "prompts", "custom.md"),
