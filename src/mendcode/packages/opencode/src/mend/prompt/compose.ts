...
async function fullKnowledge(root: string) {
  ...
  const lines = [
    ...
    "",
    "MendCode CLI map:",
    "- `mend` opens the interactive TUI. `mendcode run <message>` opens the TUI with a message queued. `mendcode chat <message>` runs a control-plane chat turn.",
    "- Health/config: `mendcode status`, `mendcode doctor`, `mendcode check`, `mendcode setup status|plan|doctor`, `mendcode config show|paths`.",
    ...
    "- Prompt/runtime: `mendcode prompts sources|build|mode|cycle-mode`, `mendcode runtime status|configure|plan|adopt|registry`, `mendcode adapter status`, `mendcode upstream status|inspect|baseline`.",
    "- Memory: `mendcode memory status|search|preview|add|edit|delete|propose|list|apply|reject|import-codex|index|config`.",
    "- Project controls: `mendcode tui status|profile|apply|preview|propose`, `mendcode focus status|list|show|use`, `mendcode packages status|list|create|install|enable|disable|remove|search|show`.",
    ...
    "- Use `mendcode memory add` only when the user explicitly asks to save/remember a fact.",
    "- Use `mendcode memory propose` for durable future-use candidates that still need approval.",
