...
/** Marker under {@link Global.Path.data}; distinct from sqlite filenames (see ADR storage migration). */
export const JSON_STORAGE_MIGRATION_DONE_BASENAME = ".mendcode-json-storage-migration-v0.done"

export function jsonStorageMigrationDonePath() {
  return path.join(Global.Path.data, JSON_STORAGE_MIGRATION_DONE_BASENAME)
}

/** Call after startup json→sqlite migration finishes successfully (including intentional skip). */
export async function writeJsonStorageMigrationDoneMarker() {
