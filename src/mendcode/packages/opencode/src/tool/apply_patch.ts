...
export const Parameters = Schema.Struct({
  patchText: Schema.String.annotate({
    description:
      "The full patch text, starting with '*** Begin Patch' and ending with '*** End Patch'. Put the patch directly in this string, not inside JSON, shell commands, or code fences. For updates, use small hunks against the exact latest file contents; if a verification error says expected lines were not found, read the file again and retry with fresher, narrower context.",
  }),
})

function verificationError(error: unknown) {
  const message = error instanceof Error ? error.message : String(error)
  if (message.includes("Failed to find expected lines") || message.includes("Failed to find context")) {
    return new Error(
      `apply_patch verification failed: ${message}\n\nRecovery: read the current file contents around the target lines, then retry with a smaller hunk that matches the latest text exactly. Do not reuse stale offset snippets or large copied blocks.`,
    )
  }
  return new Error(`apply_patch verification failed: ${message}`)
}
