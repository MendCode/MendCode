...
import {
  ...
  PTY_CONNECT_TOKEN_HEADER_LEGACY,
  ...
  return new Hono()
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Get a list of all active pseudo-terminal (PTY) sessions managed by MendCode.",
    ...
    .post(
      ...
      async (c) => {
        if (
          (c.req.header(PTY_CONNECT_TOKEN_HEADER) ?? c.req.header(PTY_CONNECT_TOKEN_HEADER_LEGACY)) !==
            PTY_CONNECT_TOKEN_HEADER_VALUE ||
          !validOrigin(c, opts)
        )
