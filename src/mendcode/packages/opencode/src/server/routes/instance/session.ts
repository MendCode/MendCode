  ...
  new Hono()
    .get(
      ...
      describeRoute({
        ...
        description: "Get a list of all MendCode sessions, sorted by most recently updated.",
    ...
    .get(
      ...
      describeRoute({
        ...
        operationId: "session.background.list",
        responses: {
          200: {
            description: "List of background sessions",
            content: {
              "application/json": {
      ...
      async (c) =>
        jsonRequest("SessionRoutes.background.list", c, function* () {
    ...
    .post(
      ...
      describeRoute({
        summary: "Register background session",
        description: "Mark a session as background-managed for Agent View.",
        operationId: "session.background.register",
        responses: {
          200: {
            description: "Background session metadata",
            content: {
              "application/json": {
                schema: resolver(BackgroundSession.Info.zod),
              },
            },
          },
          ...errors(400, 404),
      ...
      validator(
        "json",
        z.object({
          state: BackgroundSession.State.zod.optional(),
      ...
      async (c) =>
        jsonRequest("SessionRoutes.background.register", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          const session = yield* Session.Service
          const background = yield* BackgroundSession.Service
          yield* session.get(sessionID)
          return yield* background.register({ sessionID, ...body })
        }),
    )
    .delete(
      "/:sessionID/background",
      describeRoute({
        summary: "Remove background session",
        description: "Remove a session from the background Agent View registry.",
        operationId: "session.background.remove",
        responses: {
          200: {
            description: "Removed background session",
            content: {
              "application/json": {
      ...
      async (c) =>
        jsonRequest("SessionRoutes.background.remove", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const background = yield* BackgroundSession.Service
          yield* background.remove(sessionID)
          return true
        }),
    )
    .post(
          ...
          200: {
            ...
            content: {
              "application/json": {
                schema: resolver(
                  z.union([
                    z.object({ acquired: z.literal(true), info: BackgroundSession.Info.zod }),
                    z.object({ acquired: z.literal(false), info: BackgroundSession.Info.zod.optional() }),
                  ]),
                ),
              },
            },
      ...
      async (c) =>
        jsonRequest("SessionRoutes.background.writer.acquire", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          const background = yield* BackgroundSession.Service
          return yield* background.acquireWriter({ sessionID, ...body })
        }),
    )
    .delete(
      "/:sessionID/background/writer",
      ...
      async (c) =>
        jsonRequest("SessionRoutes.background.writer.release", c, function* () {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          const background = yield* BackgroundSession.Service
          return yield* background.releaseWriter({ sessionID, clientID: body.clientID })
        }),
    )
    .get(
      "/:sessionID/children",
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Create a new MendCode session for interacting with AI assistants and managing conversations.",
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Session sharing is disabled in MendCode builds.",
      ...
      async (c) =>
        jsonRequest("SessionRoutes.share", c, function* () {
          if (process.env.MENDCODE === "1") {
            throw new NamedError.Unknown({
              message: "Session sharing is disabled in MendCode.",
            })
          }
          const sessionID = c.req.valid("param").sessionID
    ...
    .delete(
      "/:sessionID/share",
      describeRoute({
        summary: "Disabled session sharing removal",
        description: "Session sharing is disabled in MendCode builds.",
