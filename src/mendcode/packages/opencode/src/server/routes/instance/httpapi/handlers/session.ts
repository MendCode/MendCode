...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const agentCommandPatch = Effect.fn("SessionHttpApi.agentCommandPatch")(function* (ctx: {
      ...
      yield* SessionError.mapStorageNotFound(session.get(ctx.params.sessionID))
      const updated = yield* agentCommandSvc
      ...
      if (updated.type === "peer_message" && updated.state === "accepted") {
        yield* promptSvc.wakePeerDelivery(updated.targetSessionID)
      }
      return updated
    })
    ...
    const backgroundRegister = Effect.fn("SessionHttpApi.backgroundRegister")(function* (ctx: {
      params: { sessionID: SessionID }
      payload: typeof BackgroundRegisterPayload.Type
    }) {
      yield* SessionError.mapStorageNotFound(session.get(ctx.params.sessionID))
      return yield* backgroundSvc.register({ sessionID: ctx.params.sessionID, ...ctx.payload })
    })

    const backgroundRemove = Effect.fn("SessionHttpApi.backgroundRemove")(function* (ctx: {
      params: { sessionID: SessionID }
    }) {
      yield* backgroundSvc.remove(ctx.params.sessionID)
      return true
    })

    const backgroundWriterAcquire = Effect.fn("SessionHttpApi.backgroundWriterAcquire")(function* (ctx: {
      params: { sessionID: SessionID }
      payload: typeof BackgroundWriterPayload.Type
    }) {
      return yield* backgroundSvc.acquireWriter({ sessionID: ctx.params.sessionID, ...ctx.payload })
    })

    const backgroundWriterRelease = Effect.fn("SessionHttpApi.backgroundWriterRelease")(function* (ctx: {
      params: { sessionID: SessionID }
    ...
    const share = Effect.fn("SessionHttpApi.share")(function* (ctx: { params: { sessionID: SessionID } }) {
      if (process.env.MENDCODE === "1") yield* Effect.fail(new HttpApiError.BadRequest({}))
    ...
    return handlers
      ...
      .handle("backgroundWriterRelease", backgroundWriterRelease)
