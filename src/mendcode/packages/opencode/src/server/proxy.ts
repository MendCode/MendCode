...
import { HEADER_PROXY_URL, HEADER_PROXY_URL_LEGACY } from "./shared/header-names"
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header(HEADER_PROXY_URL) ?? c.req.header(HEADER_PROXY_URL_LEGACY)
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set(HEADER_PROXY_URL, ProxyUtil.websocketTargetURL(target))
