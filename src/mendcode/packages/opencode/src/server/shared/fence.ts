import { HEADER_SYNC, HEADER_SYNC_LEGACY } from "@/server/shared/header-names"
...
export const HEADER = HEADER_SYNC
...
export function parse(headers: Headers) {
  const raw = headers.get(HEADER) ?? headers.get(HEADER_SYNC_LEGACY)
  if (!raw) return
