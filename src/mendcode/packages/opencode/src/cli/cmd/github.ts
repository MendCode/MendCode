...
function mendcodeGitHubDisabled() {
  return process.env.MENDCODE === "1"
    ? "The donor GitHub agent is disabled in MendCode. It installs the OpenCode GitHub App/workflow and must not be used for MendCode public releases until a MendCode-owned GitHub App/action exists."
    : null
}
...
export const GithubInstallCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.install")(function* () {
    ...
    if (disabled) return yield* fail(disabled)
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://mendcode.ai/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.mendcode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.mendcode.ai" : "https://mendcode.ai"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.mendcode.ai"
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/mendcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
