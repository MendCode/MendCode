...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run MendCode runtime",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found donor session: ${sessionUrl}`)

          if (process.env.MENDCODE === "1") {
            UI.println("Skipping donor session import in MendCode mode.")
          } else {
    ...
    UI.println()
    UI.println("Starting MendCode runtime...")
    UI.println()
    ...
    if (code !== 0) return yield* Effect.die(new Error(`MendCode runtime exited with code ${code}`))
  }),
})
