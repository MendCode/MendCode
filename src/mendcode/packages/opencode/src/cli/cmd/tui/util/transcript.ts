...
import { formatAssistantUsage } from "./usage"
...
export function formatAssistantHeader(
  ...
  const usage = formatAssistantUsage(msg, providers)?.tokens

  return `## Assistant (${[Locale.titlecase(msg.agent), modelName, usage, duration].filter(Boolean).join(" · ")})\n\n`
}
