    ...
    <Switch>
      ...
      <Match when={store.stage === "permission"}>
        {(() => {
          ...
          const current = info()
          const questionTitle = () => {
            if (current.title.startsWith("Edit ")) return `Allow Edit to ${current.title.slice("Edit ".length)}?`
            return `Allow ${current.title}?`
          }
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              <box flexDirection="row" gap={1} paddingLeft={1} flexShrink={0}>
                ...
                <text fg={theme.text}>{questionTitle()}</text>
              </box>
            </box>
          )
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell MendCode what to do differently</text>
        </box>
      </box>
      <box
...
function Prompt<const T extends Record<string, string>>(props: {
  ...
  const content = () => (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={0} flexGrow={1}>
        <Show
          when={props.header}
          fallback={
            <box flexDirection="row" gap={1} paddingLeft={1} flexShrink={0}>
      ...
      >
        <box flexDirection="column" gap={0} width="100%">
          <For each={keys}>
            {(option, index) => (
              ...
              >
                <text fg={option === store.selected ? theme.text : theme.textMuted} flexShrink={0} width={2}>
                  {String(index() + 1)}
                </text>
                <text fg={option === store.selected ? theme.text : theme.textMuted} flexShrink={0}>
                  {option === store.selected ? "(●)" : "(○)"}
                </text>
                <text fg={option === store.selected ? theme.text : theme.textMuted} wrapMode="none">
        ...
        <box flexDirection={narrow() ? "column" : "row"} gap={2} flexShrink={0}>
