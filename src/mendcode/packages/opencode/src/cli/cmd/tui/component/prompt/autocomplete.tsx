...
export function Autocomplete(props: {
  ...
  function hide() {
    ...
    const slashText = text.slice(store.index, props.input().cursorOffset)
    if (store.visible === "/" && slashText.startsWith("/") && !slashText.endsWith(" ") && !slashText.match(/\s/)) {
      const cursor = props.input().logicalCursor
      const previousOffset = props.input().cursorOffset
      props.input().cursorOffset = store.index
      const start = props.input().logicalCursor
      ...
      onInput(value) {
        if (store.visible) {
          if (
            ...
            // "/<command>" already has arguments after it
            (store.visible === "/" &&
              props
                .input()
        ...
        const slashTrigger = findSlashAutocompleteTrigger(value, offset)
        if (slashTrigger) {
          show("/")
          setStore("index", slashTrigger.index)
          return
        }
