...
import mendcodeTheme from "./theme/mendcode.json" with { type: "json" }
...
import { profileTheme } from "@/mend/profile"
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const mendThemeName = mend.profile.theme.palette || "mend-dark"
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", mendThemeName)
        draft.active = typeof active === "string" ? active : mendThemeName
        draft.ready = false
      }),
    )

    createEffect(() => {
      customThemes[mendThemeName] = profileTheme(mend.profile)
      syncThemes()
    })
      ...
      void Promise.allSettled([
        ...
        getCustomThemes()
          .then((custom) => {
            customThemes = {
              ...custom,
              [mendThemeName]: profileTheme(mend.profile),
            }
            syncThemes()
          })
          .catch(() => {
            setStore("active", mendThemeName)
          }),
      ]).finally(() => {
        setStore("ready", true)
      })
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes[mendThemeName] ?? store.themes.mendcode ?? store.themes.mendcode, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const diffGreen = RGBA.fromHex(isDark ? "#3fb950" : "#1a7f37")
  const diffAddedBg = tint(bg, diffGreen, diffAlpha)
  const diffRemovedBg = tint(bg, ansiColors.red, diffAlpha)
  const diffContextBg = grays[2]
  const diffAddedLineNumberBg = tint(diffContextBg, diffGreen, diffAlpha)
