  ...
  return (
    <box flexDirection="row" justifyContent="space-between" gap={1} flexShrink={0}>
      ...
      <box gap={2} flexDirection="row" flexShrink={0}>
        <Show when={mend.profile.layout.zones.footer.enabled}>
          <text fg={theme.textMuted}>
            {mend.profile.identity.productName} · {mend.profile.theme.palette} · {mend.profile.layout.density}
          </text>
        </Show>
        <Switch>
          <Match when={connectionLabel()}>{(label) => <text fg={connectionColor()}>{label()}</text>}</Match>
