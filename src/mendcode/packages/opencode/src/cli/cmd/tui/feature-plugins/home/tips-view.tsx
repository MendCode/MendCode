...
const TIPS = [
  ...
  "Place TUI settings in {highlight}~/.config/mendcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "MendCode auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.mendcode/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.mendcode/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "MendCode auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "MendCode uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.mendcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.mendcode/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent MendCode from reading sensitive files",
  "Use {highlight}mendcode run{/highlight} for non-interactive scripting",
  "Use {highlight}mendcode run --continue{/highlight} to resume the last session",
  "Use {highlight}mendcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}mend runtime status{/highlight} to inspect the adopted MendCode runtime",
  "Run {highlight}mend runtime plan{/highlight} before changing owned runtime internals",
  "Run {highlight}mend check{/highlight} to validate the MendCode harness state",
  "Run {highlight}mendcode providers status{/highlight} to see all configured providers",
  "Run {highlight}mendcode auth status openai{/highlight} to inspect OpenAI auth status",
  "Use GitHub workflow comments only after your MendCode automation is configured",
  "Run {highlight}mendcode upstream status{/highlight} before changing donor-derived automation flows",
  "Comment {highlight}/review{/highlight} on PR code lines for targeted code reviews",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.mendcode/themes/{/highlight} directory",
  ...
  "Run {highlight}mendcode status{/highlight} to troubleshoot configuration and project state",
  ...
  "Containerized MendCode packaging is not finalized yet; use the local runtime for now",
  "Use {highlight}/connect{/highlight} to configure curated provider access inside MendCode",
  "Commit your project's {highlight}AGENTS.md{/highlight} file to Git for team sharing",
  "Use {highlight}/review{/highlight} to review uncommitted changes, branches, or PRs",
  "Run {highlight}/help{/highlight} or {highlight}Ctrl+X H{/highlight} to show the help dialog",
  "Use {highlight}/rename{/highlight} to rename the current session",
   "Press {highlight}Ctrl+_{/highlight} to undo and {highlight}Ctrl+Y{/highlight} to redo changes in your prompt",
]
