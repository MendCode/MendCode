...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old mendcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.mendcode` /
  // `.mendcode` folders between cwd and home, and OPENCODE_CONFIG_DIR /
  // MENDCODE_CONFIG_DIR (via Flag).
  ...
  // 2. Explicit MENDCODE_TUI_CONFIG / OPENCODE_TUI_CONFIG override, if set.
  ...
  // 4. `.mendcode` / `.mendcode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".mendcode") || dir.endsWith(".mendcode") || dir === Flag.OPENCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".mendcode") && !dir.endsWith(".mendcode") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
