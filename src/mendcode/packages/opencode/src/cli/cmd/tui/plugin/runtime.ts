...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      base === ".mendcode" || base === ".mendcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".mendcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  const mendTui = path.join(state.directory, ".mendcode", "tui.json")
  const openTui = path.join(state.directory, ".mendcode", "tui.json")
  const projectTui = existsSync(mendTui) ? mendTui : openTui
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || projectTui,
  }
}
...
async function installPluginBySpec(
  ...
  if (process.env.MENDCODE === "1" && process.env.MENDCODE_ALLOW_TUI_PLUGIN_INSTALL !== "1") {
    return {
      ok: false,
      message: "External TUI plugin installs are disabled in MendCode. Use the MendCode marketplace/registry flow instead.",
    }
  }
  if (!state) {
    return {
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  runtime = next
  try {
    // Keep TUI plugin loading client-only. Entering WithInstance here bootstraps
    // a duplicate project runtime (DB, watchers, and services) in every attached TUI.
