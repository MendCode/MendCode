import { asSchema, jsonSchema, tool, type LanguageModelMiddleware, type Tool } from "ai"
  ...
  return {
    ...
    async transformParams({ params }) {
        ...
        params.prompt.flatMap((message) =>
          message.role !== "assistant"
            ...
            : message.content.flatMap((part) =>
                part.type === "tool-call" && part.toolName === "tool_search" && !part.providerExecuted
                  ? [part.toolCallId]
                  : [],
              ),
        ),
      ...
      for (const message of params.prompt) {
        ...
        for (const part of message.content) {
          ...
          if (!part.input || typeof part.input !== "object" || Array.isArray(part.input)) {
            throw new Error(
              `Invalid stored tool_search arguments for call ${part.toolCallId}. History was preserved; fork the session before this call to recover without replaying the tool.`,
            )
          }
        }
      }
      ...
      const transformed: LanguageModelV3CallOptions = {
        ...params,
        tools: params.tools?.map((item) =>
          item.type === "function" && item.name === "tool_search" ? { ...item, name: WIRE_SEARCH } : item,
        ),
        toolChoice:
          params.toolChoice?.type === "tool" && params.toolChoice.toolName === "tool_search"
            ? { ...params.toolChoice, toolName: WIRE_SEARCH }
            : params.toolChoice,
  ...
  const matches = Object.entries(tools)
    ...
    .map(([name, value]) => {
      ...
      return {
        ...
        score: words.reduce(
          (sum, word) =>
            sum + (title === word ? 20 : title.includes(word) ? 8 : description.includes(word) ? 1 : 0),
          0,
        ),
      }
    })
  ...
  for (const message of messages) {
    for (const part of message.parts) {
      ...
      for (const name of found) {
        if (typeof name === "string" && !PRIMARY.has(name) && names.size < MAX_DISCOVERED) names.add(name)
      }
    }
  }
  return names
