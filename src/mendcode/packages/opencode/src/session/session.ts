...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".mendcode", "plans")
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service | SyncEvent.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    function appendDeltaField(part: MessageV2.Part, field: string, delta: string) {
    ...
    function schedulePendingPartDeltaFlush(key: string, pending: PendingPartDelta) {
      ...
      pending.timer = setTimeout(() => {
        ...
        pending.flush = Effect.runPromise(
          ...
          .finally(() => {
            const latest = pendingPartDeltas.get(key)
            ...
            schedulePendingPartDeltaFlush(key, pending)
          })
      }, LIVE_PART_DELTA_PERSIST_INTERVAL_MS)
    }

    const clearPendingPartDelta = (part: MessageV2.Part) =>
      Effect.promise(async () => {
        const key = partDeltaKey({ sessionID: part.sessionID, messageID: part.messageID, partID: part.id })
    ...
    const updatePartDelta = Effect.fnUntraced(function* (input: {
      ...
      const key = partDeltaKey(input)
      let pending = pendingPartDeltas.get(key)
      if (!pending) {
        ...
        pendingPartDeltas.set(key, pending)
      }
      if (!appendDeltaField(pending.part, input.field, input.delta)) return
      pending.version++
      schedulePendingPartDeltaFlush(key, pending)
    })
...
function directoryScopeCondition(directory: string) {
  const normalized = directory.replace(/\/+$/, "")
  if (!normalized) return eq(SessionTable.directory, directory)
  return or(eq(SessionTable.directory, normalized), like(SessionTable.directory, `${normalized}/%`))!
}
...
function* listByProject(
  ...
  if (input.path !== undefined) {
    ...
    if (input.directory) {
      conditions.push(directoryScopeCondition(input.directory))
    }
  }
  if (input.roots) {
