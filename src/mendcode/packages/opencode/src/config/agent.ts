...
function normalizeAgentFileName(name: string) {
  return name.replace(/^(?:sub|subagent)[/-]+/i, "")
}
...
export async function load(dir: string) {
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    const patterns = ["/.mendcode/agent/", "/.mendcode/agents/", "/agent/", "/agents/"]
    const name = normalizeAgentFileName(configEntryNameFromPath(item, patterns))
