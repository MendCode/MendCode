...
import { InstallationLocal, InstallationVersion } from "@mendcode/core/installation/version"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  if ("theme" in copy || "keybinds" in copy || "tui" in copy) {
    mutated = true
    delete copy.theme
  ...
  if (removedControlPlaneKeys.length) {
    mutated = true
    for (const key of removedControlPlaneKeys) delete copy[key]
    log.debug("ignoring MendCode control-plane config keys in runtime config loader", {
      path: source,
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for MendCode serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration for slash commands and command files",
  }),
  ...
  subagent_model: Schema.optional(ConfigModelID).annotate({
    description: "Default model to use for subagents launched by the task tool in the format of provider/model",
  }),
  ...
  plan_exit_agent: Schema.optional(Schema.String).annotate({
    description:
      "Primary agent to switch to after plan_exit or plan_review approval. Defaults to 'build'. Must not be a subagent or hidden agent.",
  }),
...
function globalConfigFile() {
  const candidates = ["mendcode.jsonc", "mendcode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://mendcode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mendcode.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mendcode.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://mendcode.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://mendcode.ai/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          ...
          for (const file of yield* ConfigPaths.files("mendcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
            yield* merge(file, yield* loadFile(file), "local")
          }
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.mendcode/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (existsSync(managedDir)) {
          for (const file of ["mendcode.json", "mendcode.jsonc"]) {
