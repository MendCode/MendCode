---
title: ACP-støtte
description: Bruk MendCode i en hvilken som helst ACP-kompatibel editor.
---

MendCode støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke MendCode via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `mend acp`.

Kommandoen starter MendCode som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'MendCode'-agenten i AI Chat-agentvelgeren.
...
For å bruke MendCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke MendCode som ACP-agent for chat.
...
MendCode fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i MendCode-konfigurasjonen
