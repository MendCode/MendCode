...
Du kan legge til eksterne verktøy i MendCode ved å bruke _Model Context Protocol_ eller MCP. MendCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [MendCode Config](/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="mendcode.jsonc" {6}
{
...
MendCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil MendCode:
...
Hvis serveren krever autentisering, vil MendCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `mend mcp auth <server-name>`.
...
```bash
mend mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil MendCode lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
mend mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i MendCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom MendCode-konfigurasjonen som alle andre verktøy.
...
```json title="mendcode.json" {11, 14-18}
{
  "$schema": "https://MendCode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
mend mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble MendCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver MendCode-samtale oppført som en separat økt.
