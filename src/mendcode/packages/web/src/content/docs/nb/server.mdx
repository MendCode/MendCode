---
title: Tjener
description: Samhandle med MendCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `mend serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en MendCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `MendCode.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `MendCode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `mend serve` og `mend web`.
...
Når du kjører `MendCode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk MendCode-serveren til å samhandle med MendCode programmatisk.
:::

Denne arkitekturen lar MendCode støtte flere klienter og lar deg samhandle med MendCode programmatisk.

Du kan kjøre `mend serve` for å starte en frittstående server. Hvis du har
MendCode TUI kjører, vil `mend serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av MendCode [IDE](/docs/ide) plugins.
...
MendCode-serveren viser følgende APIer.
