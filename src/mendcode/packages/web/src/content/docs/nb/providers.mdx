---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i MendCode-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i MendCode
konfig.
...
MendCode Zen er en liste over modeller levert av MendCode-teamet som har vært
testet og verifisert for å fungere godt med MendCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med MendCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `MendCode Zen` og gå til [MendCode console](/auth).
...
Det fungerer som alle andre leverandører i MendCode og er helt valgfritt å bruke.

---

## MendCode Go

MendCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av MendCode-teamet som har vært
testet og verifisert for å fungere godt med MendCode.

1. Kjør kommandoen `/connect` i TUI, velg `MendCode Go`, og gå til [MendCode console](/auth).
...
Slik bruker du Amazon Bedrock med MendCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører MendCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `mendcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i MendCode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere MendCode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at MendCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom MendCode-konfigurasjonen.
...
for å lagre token i MendCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
MendCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse MendCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`mendcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `mendcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med MendCode:
...
Slik bruker du Google Vertex AI med MendCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører MendCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom MendCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session) plugin for automatisk å logge hver MendCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver MendCode-samtale oppført som en separat økt.
...
Du kan konfigurere MendCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere MendCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere MendCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for MendCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/MendCode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med MendCode:
...
3. Kopier API-nøkkelen for bruk i MendCode.
...
6. **Viktig**: Før du bruker skymodeller i MendCode, må du hente modellinformasjonen lokalt:
...
MendCode Zen er en liste over testede og verifiserte modeller levert av MendCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>MendCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **MendCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn MendCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom MendCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom MendCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MendCode:
...
Du kan også tilpasse modeller gjennom MendCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med MendCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `mendcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar MendCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `mend auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk MendCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i MendCode-konfigurasjonen.
