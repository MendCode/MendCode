---
title: Regler
description: Set custom instructions for MendCode.
---

Du kan gi tilpassede instruksjoner for å MendCode ved å opprette en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for å tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialisering

For å lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i MendCode.

:::tip
Du bør sjekke inn prosjektets `AGENTS.md`-fil i Git.
:::

Dette vil skanne prosjektet og alt dets innhold for å forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper MendCode å navigere i prosjektet bedre.
...
Du kan også ha globale regler i en `~/.config/opencode/AGENTS.md`-fil. Dette blir brukt på alle MendCode-økter.
...
For brukere som migrerer fra Claude Code, støtter MendCode Claude Codes filkonvensjoner som fallbacks:
...
Du kan spesifisere egendefinerte instruksjonsfiler i din `mendcode.json` eller den globale `~/.config/opencode/mendcode.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
### Bruker mendcode.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `mendcode.json`:
...
Du kan lære MendCode å lese eksterne filer ved å gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørge for at MendCode laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er bruk av `mendcode.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
