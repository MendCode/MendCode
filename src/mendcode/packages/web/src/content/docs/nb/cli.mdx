---
title: CLI
description: MendCode CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
mend
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med MendCode programmatisk.
...
Start MendCode-terminalbrukergrensesnittet.
...
Administrer agenter for MendCode.
...
Koble en terminal til en allerede kjørende MendCode-backend-server startet via `serve` eller `web` kommandoer.

```bash
mend attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern MendCode-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `mend`) |
...
MendCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `mend auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

Når MendCode starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i MendCode.
...
Kjør MendCode i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `mend serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende MendCode-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `mend`) |
...
Start en headless MendCode-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
mend serve
```

Dette starter en HTTP-server som gir API tilgang til MendCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `mend`).
...
Vis tokenbruk og kostnadsstatistikk for MendCode-øktene dine.
...
Importer øktdata fra en JSON-fil eller en MendCode share-URL.
...
Start en headless MendCode-server med et webgrensesnitt.

```bash
mend web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til MendCode via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `mend`).
...
Avinstaller MendCode og fjern alle relaterte filer.
...
Oppdaterer MendCode til siste versjon eller en spesifikk versjon.
...
MendCode CLI bruker følgende globale flagg.
...
MendCode kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `mend`) |
