...
Det er to typer agenter i MendCode; primære agenter og underagenter.
...
MendCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
MendCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

MendCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `mendcode.json`:
...
Hvis ingen temperatur er spesifisert, bruker MendCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale MendCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i MendCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [MendCode Zen](/docs/zen), vil du bruke `MendCode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `mend models` for å se en liste over tilgjengelige modeller.
:::
