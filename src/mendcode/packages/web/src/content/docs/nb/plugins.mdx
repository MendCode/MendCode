...
description: Skriv dine egne programtillegg for å utvide MendCode.
---

Plugins lar deg utvide MendCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til MendCode.
...
- `.mendcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/MendCode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/opencode/mendcode.json`)
2. Prosjektkonfigurasjon (`mendcode.json`)
3. Global plugin-katalog (`~/.config/opencode/plugins/`)
4. Prosjektpluginkatalog (`.mendcode/plugins/`)
...
MendCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En MendCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide MendCode.
...
:::note
Hvis du bruker MendCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre MendCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til MendCode:
...
`tool`-hjelperen lager et tilpasset verktøy som MendCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for MendCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](/docs/sdk) for detaljer.
