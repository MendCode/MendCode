---
title: Pravila
description: Postavite prilagodena uputstva za MendCode.
---

Mozete dodati prilagodena uputstva za MendCode tako sto kreirate `AGENTS.md` datoteku. Ovo je slicno pravilima u Cursor-u. Sadrzi uputstva koja se ubacuju u LLM kontekst da prilagode ponasanje za vas projekat.

---

## Inicijalizacija

Da kreirate novu `AGENTS.md` datoteku, pokrenite `/init` komandu u MendCode.

:::tip
Preporuceno je da `AGENTS.md` iz projekta commitujete u Git.
:::

Ovo skenira projekat i njegov sadrzaj, razumije cemu projekat sluzi i generise `AGENTS.md`. Tako MendCode bolje navigira kroz kod.
...
MendCode podrzava citanje `AGENTS.md` datoteke sa vise lokacija. Svaka lokacija ima drugu svrhu.
...
Mozete imati i globalna pravila u `~/.config/opencode/AGENTS.md`. Ona se primjenjuju u svim MendCode sesijama.
...
Za korisnike koji prelaze sa Claude Code, MendCode podrzava i Claude konvencije datoteka kao rezervu:
...
Kada se MendCode pokrene, trazi datoteke pravila ovim redoslijedom:
...
Mozete navesti prilagodene datoteke uputstava u `mendcode.json` ili globalnom `~/.config/opencode/mendcode.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
Iako MendCode ne parsira automatski reference datoteka u `AGENTS.md`, slicno ponasanje mozete dobiti na dva nacina:

### Korištenje mendcode.json

Preporuceni pristup je da koristite `instructions` polje u `mendcode.json`:
...
Mozete nauciti MendCode da cita eksterne datoteke tako sto cete dati eksplicitna uputstva u `AGENTS.md`. Evo prakticnog primjera:
...
- Osigurate da MendCode ucitava datoteke samo kad su potrebne za konkretan zadatak

:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `mendcode.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
