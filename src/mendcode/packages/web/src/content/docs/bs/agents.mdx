...
Postoje dvije vrste agenata u MendCode: primarni agenti i podagenti.
...
MendCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
MendCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

MendCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `mendcode.json`:
...
Ako temperatura nije navedena, MendCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu MendCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj MendCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [MendCode Zen](/docs/zen), koristili biste `MendCode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `mend models` da vidite listu dostupnih modela.
:::
