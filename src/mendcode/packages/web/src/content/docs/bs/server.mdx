---
title: Server
description: Komunicirajte s MendCode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `mend serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji MendCode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `MendCode.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `MendCode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `mend serve` i za `mend web`.
...
Kada pokrenete `MendCode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite MendCode server za programsku interakciju sa MendCode-om.
:::

Ova arhitektura omogucava MendCode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `mend serve` da startate standalone server. Ako je MendCode TUI vec pokrenut, `mend serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste MendCode [IDE](/docs/ide) pluginovi.
...
MendCode server izlaže sljedece API-je.
