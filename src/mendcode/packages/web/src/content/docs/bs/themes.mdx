...
U MendCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, MendCode koristi nasu `MendCode` temu.
...
MendCode dolazi sa vise ugradenih tema.
...
- Zele da MendCode odgovara izgledu njihovog terminala
...
MendCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
3. **Korijenski direktorij projekta** - `<project-root>/.mendcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.mendcode/themes/*.json`
