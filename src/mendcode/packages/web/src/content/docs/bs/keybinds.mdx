...
MendCode ima listu veza tipki koje možete prilagoditi putem `tui.json`.
...
MendCode koristi `leader` (vodeću) tipku za većinu povezivanja tipki. Ovo izbjegava sukobe u vašem terminalu.
...
Unos prompta aplikacije MendCode za desktop podržava uobičajene prečice u stilu Readline/Emacs za uređivanje teksta. One su ugrađene i trenutno se ne mogu konfigurirati putem `mendcode.json`.
