---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj MendCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem MendCode
config.
...
MendCode Zen je lista modela koje je obezbedio MendCode tim koji su bili
testirano i potvrđeno da dobro radi sa MendCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa MendCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite MendCode i idite na [MendCode.ai/auth](https://MendCode.ai/auth).
...
Radi kao i svaki drugi provajder u MendCode i potpuno je opcionalan za korištenje.

---

## MendCode Go

MendCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža MendCode tim i koji su testirani i verificirani da dobro rade s MendCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `MendCode Go` i idite na [MendCode console](/auth).
...
Da biste koristili Amazon Bedrock s MendCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete MendCode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `mendcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u MendCode.
:::
...
Možete konfigurirati MendCode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi MendCode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju MendCode konfiguraciju.
...
da pohrani token u mend auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
MendCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate MendCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`mendcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `mendcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s MendCode:
...
Za korištenje Google Vertex AI s MendCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće MendCode.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session) da automatski prijavite svaki MendCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki MendCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati MendCode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati MendCode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati MendCode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za MendCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/MendCode) za detalje.
:::
...
Da biste koristili Ollama Cloud s MendCode:
...
3. Kopirajte API ključ za korištenje u MendCode.
...
6. **Važno**: Prije upotrebe modela oblaka u MendCode, morate lokalno povući informacije o modelu:
...
MendCode Zen je lista testiranih i verifikovanih modela koju je obezbedio MendCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>MendCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **MendCode Zen**.

```txt
   /connect
```

3. Unesite svoj MendCode API ključ.
...
Također možete dodati dodatne modele putem vaše MendCode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše MendCode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju MendCode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s MendCode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `mendcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju MendCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `mend auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite MendCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj MendCode konfiguraciji.
