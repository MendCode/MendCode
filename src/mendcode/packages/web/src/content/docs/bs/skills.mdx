...
Agent skills omogucavaju MendCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
MendCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.mendcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, MendCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.mendcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.
...
Kreirajte `.mendcode/skills/git-release/SKILL.md` ovako:
...
MendCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `mendcode.json`:
...
**Za ugradene agente** (u `mendcode.json`):
