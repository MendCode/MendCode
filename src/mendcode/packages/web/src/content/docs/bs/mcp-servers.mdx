...
Možete dodati vanjske alate u MendCode koristeći _Model Context Protocol_, ili MCP. MendCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [MendCode Config](/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
MendCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, MendCode će:
...
Ako server zahtijeva autentifikaciju, MendCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `mend mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, MendCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u MendCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz MendCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete MendCode sa vašim Sentry nalogom.
