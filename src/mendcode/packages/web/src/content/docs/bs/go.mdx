...
MendCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
MendCode Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u MendCode-u. Pretplatite se na MendCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili MendCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa MendCode-om.
:::
...
MendCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

MendCode Go radi kao bilo koji drugi provajder u MendCode-u.

1. Prijavite se na **<a href={console}>MendCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `MendCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na MendCode Go.
:::
...
MendCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj MendCode konfiguraciji
koristi format `MendCode-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`MendCode-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo MendCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz MendCode koristite i bilo kojeg drugog provajdera.
