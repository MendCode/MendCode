---
title: CLI
description: MendCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
mend
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa MendCode.
...
Pokrenite MendCode terminalski korisnički interfejs.
...
MendCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte MendCode agentima.
...
Priključite terminal na već pokrenut MendCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
mend attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim MendCode backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `mend`) |
...
MendCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `mend auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

Kada se MendCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u MendCode.
...
Pokrenite MendCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `mend serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti MendCode server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `mend`) |
...
Pokrenite MendCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
mend serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti MendCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `mend`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše MendCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili MendCode dijeljenog URL-a.

```bash
mend import <file>
```

Možete uvesti iz lokalne datoteke ili MendCode dijeljenog URL-a.
...
Pokrenite MendCode headless server sa web interfejsom.

```bash
mend web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup MendCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `mend`).
...
Deinstalirajte MendCode i uklonite sve povezane datoteke.
...
Ažurira MendCode na najnoviju verziju ili određenu verziju.
...
MendCode CLI prihvata sljedeće globalne zastavice.
...
MendCode se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `mend`) |
