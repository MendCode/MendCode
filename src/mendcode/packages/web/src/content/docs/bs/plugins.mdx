---
title: Dodaci
description: Napišite vlastite dodatke za proširenje MendCode.
---

Dodaci vam omogućavaju da proširite MendCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja MendCode.
...
- `.mendcode/plugins/` - Dodaci na nivou projekta
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/MendCode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/opencode/mendcode.json`)
2. Konfiguracija projekta (`mendcode.json`)
3. Globalni direktorij dodataka (`~/.config/opencode/plugins/`)
4. Direktorij dodataka projekta (`.mendcode/plugins/`)
...
MendCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: MendCode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje MendCode.
...
Spriječite MendCode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u MendCode:
...
Pomoćnik `tool` kreira prilagođeni alat koji MendCode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za MendCode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](/docs/sdk) za detalje.
