---
title: Regler
description: Angiv tilpassede instruktioner til MendCode.
---

Du kan gi tilpassede instruksjoner for at MendCode ved at oprete en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for at tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialisering

For at lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i MendCode.

:::tip
Du bør overgi prosjektets `AGENTS.md`-fil til Git.
:::

Dette vil skanne prosjektet og alt dets innhold for at forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper MendCode at navigere i prosjektet bedre.
...
MendCode støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/opencode/AGENTS.md`-fil. Dette blir brugt på alle MendCode-sessioner.
...
For brugere som migrerer fra Claude Code, støtter MendCode Claude Codes filkonvensjoner som fallbacks:
...
Når MendCode starter, ser den etter regelfiler i denne rekkefølgen:
...
Du kan spesifisere egendefinerte instruksjonsfiler i din `mendcode.json` eller den globale `~/.config/opencode/mendcode.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
Selv om MendCode ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:

### Brug af mendcode.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `mendcode.json`:
...
Du kan lære MendCode at lese eksterne filer ved at gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at MendCode laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er brug av `mendcode.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
