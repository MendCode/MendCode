...
Der er to typer agenter i MendCode; primære agenter og subagenter.
...
MendCode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
MendCode leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

MendCode leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `mendcode.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger MendCode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale MendCode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din MendCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [MendCode Zen](/docs/zen), vil du bruge `MendCode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `mend models` for at se en liste over de tilgængelige modeller.
:::
