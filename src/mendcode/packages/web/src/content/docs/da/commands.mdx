...
Opret `.mendcode/commands/test.md`:
...
Du kan tilføje brugerdefinerede kommandoer gennem MendCode-konfigurationen eller ved at oprette markdown-filer i `commands/`-mappen.

---

### JSON

Brug indstillingerne `command` i din MendCode [config](/docs/config):
...
MendCode indeholder flere indbyggede kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
