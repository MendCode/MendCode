...
Agentferdigheter lar MendCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
MendCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.mendcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går MendCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.mendcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.
...
Lag `.mendcode/skills/git-release/SKILL.md` slik:
...
MendCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `mendcode.json`:
...
**For innebygde agenter** (i `mendcode.json`):
