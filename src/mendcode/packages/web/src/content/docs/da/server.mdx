---
title: Server
description: Interager med MendCode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `mend serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en MendCode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `MendCode.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `MendCode`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `mend serve` og `mend web`.
...
Når du kører `MendCode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug MendCode-serveren til at interagere med MendCode programmatisk.
:::

Denne arkitektur lader MendCode understøtte flere klienter og lader dig interagere med MendCode programmatisk.

Du kan køre `mend serve` for at starte en selvstændig server. Hvis du har
MendCode TUI kørende, vil `mend serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af MendCode [IDE](/docs/ide) plugins.
...
MendCode-serveren viser følgende API'er.
