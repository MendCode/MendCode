...
description: Brug MendCode i enhver ACP-kompatibel editor.
---

MendCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge MendCode via ACP, konfigurer din editor til at køre kommandoen `mend acp`.

Kommandoen starter MendCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'MendCode'-agent i AI Chat-agentvælgeren.
...
For at bruge MendCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge MendCode som ACP-agent til chat.
...
MendCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din MendCode-konfiguration
