---
title: Plugins
description: Skriv dine egne plugins for at udvide MendCode.
---

Plugins giver dig mulighed for at udvide MendCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre MendCodes standardadfærd.
...
- `.mendcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/MendCode/node_modules/`.
...
1. Global konfiguration (`~/.config/opencode/mendcode.json`)
2. Projektkonfiguration (`mendcode.json`)
3. Global plugin-mappe (`~/.config/opencode/plugins/`)
4. Projekt plugin bibliotek (`.mendcode/plugins/`)
...
MendCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En MendCode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide MendCode.
...
:::note
Hvis du bruger MendCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå MendCode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til MendCode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som MendCode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for MendCode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](/docs/sdk) for detaljer.
