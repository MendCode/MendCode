---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i MendCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i MendCode-konfigurationen.
...
MendCode Zen er en liste over modeller leveret af MendCode-teamet, som er blevet testet og verificeret til at fungere godt med MendCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med MendCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg MendCode og gå til [MendCode.ai/auth](https://MendCode.ai/auth).
...
Det fungerer som alle andre udbydere i MendCode og er helt valgfrit at bruge.

---

## MendCode Go

MendCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af MendCode-teamet, som er testet og verificeret til at fungere godt med MendCode.

1. Kør kommandoen `/connect` i TUI, vælg `MendCode Go`, og gå til [MendCode console](/auth).
...
Sådan bruger du Amazon Bedrock med MendCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører MendCode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `mendcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i MendCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere MendCode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at MendCode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem MendCode-konfigurationen.
...
gemme tokenet i mend auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
MendCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse MendCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`mendcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `mendcode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med MendCode:
...
Sådan bruger du Google Vertex AI med MendCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører MendCode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem MendCode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session) plugin for automatisk at logge hver MendCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver MendCode-samtale opført som en separat session.
...
Du kan konfigurere MendCode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere MendCode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere MendCode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for MendCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/MendCode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med MendCode:
...
3. Kopier API-nøglen til brug i MendCode.
...
6. **Vigtigt**: Før du bruger skymodeller i MendCode, skal du hente modelinformationen lokalt:
...
MendCode Zen er en liste over testede og verificerede modeller leveret af MendCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>MendCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **MendCode Zen**.

   ```txt
   /connect
   ```

3. Indtast MendCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem MendCode-konfigurationen.
...
5. Du kan også tilpasse dem gennem MendCode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MendCode:
...
Du kan også tilpasse modeller gennem MendCode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med MendCode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `mendcode.json`-filen i projektkataloget:
...
`limit`-felterne lader MendCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `mend auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek MendCode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i MendCode-konfigurationen.
