...
For at fejlfinde problemer med MendCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\MendCode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `MendCode --log-level DEBUG`.

---

## Lagring

MendCode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\MendCode`
...
MendCode Desktop kører en lokal MendCode-server (`MendCode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `MendCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opencode/mendcode.jsonc` (eller `~/.config/opencode/mendcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/mendcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\MendCode\mendcode.jsonc`
...
MendCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\MendCode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så MendCode kan genopbygge den.

1. Afslut MendCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/MendCode`
- **Linux**: slet `~/.cache/MendCode` (eller kør `rm -rf ~/.cache/MendCode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\MendCode`

3. Start MendCode Desktop på ny.

---

### Løs problemer med serverforbindelse

MendCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `mendcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver MendCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i MendCode.

---

### Meddelelser vises ikke

MendCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for MendCode i dine OS-indstillinger, og
...
1. Afslut MendCode Desktop.
2. Find og slet disse filer (de findes i MendCode Desktop-appens datamappe):

- `MendCode.settings.dat` (skrivebordsstandardserver URL)
- `MendCode.global.dat` og `MendCode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med MendCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `mend models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\MendCode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. MendCode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\MendCode`

2. Start MendCode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge MendCode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
MendCode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
