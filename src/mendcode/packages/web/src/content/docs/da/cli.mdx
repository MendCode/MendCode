---
title: CLI
description: MendCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
mend
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med MendCode programmatisk.
...
Start MendCode terminalbrugergrænsefladen.
...
MendCode CLI har også følgende kommandoer.

---

### agent

Administrer agenter i MendCode.
...
Tilslut en terminal til en allerede kørende MendCode backend-server startet via `serve` eller `web` kommandoer.

```bash
mend attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern MendCode backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `mend`) |
...
MendCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `mend auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

Når MendCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i MendCode.
...
Kør MendCode i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `mend serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende MendCode-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `mend`)   |
...
Start en hovedløs MendCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
mend serve
```

Dette starter en HTTP-server, der giver API-adgang til MendCode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `mend`).
...
Administrator MendCode sessionsholder.
...
Liste over alle MendCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine MendCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller MendCode del URL.

```bash
mend import <file>
```

Du kan importere fra en lokal fil eller en MendCode share URL.
...
Start en hovedløs MendCode-server med en webgrænseflade.

```bash
mend web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til MendCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `mend`).
...
Afinstaller MendCode og fjern alle relaterede filer.
...
Opdaterer MendCode til den nyeste version eller en specifik version.
...
MendCode CLI tager følgende globale flag.
...
MendCode kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `mend`) |
