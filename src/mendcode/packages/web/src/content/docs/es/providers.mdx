---
title: Proveedores
description: Usando cualquier proveedor LLM en MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración MendCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu MendCode
configuración.
...
MendCode Zen es una lista de modelos proporcionados por el equipo MendCode que han sido
probado y verificado para funcionar bien con MendCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con MendCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione MendCode y diríjase a [MendCode.ai/auth](https://MendCode.ai/auth).
...
Funciona como cualquier otro proveedor en MendCode y su uso es completamente opcional.

---

## MendCode Go

MendCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de MendCode que han sido
probado y verificado para funcionar bien con MendCode.

1. Ejecute el comando `/connect` en TUI, seleccione `MendCode Go` y diríjase a [MendCode console](/auth).
...
Para usar Amazon Bedrock con MendCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta MendCode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `mendcode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en MendCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar MendCode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que MendCode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración MendCode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `MendCode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación MendCode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
MendCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear MendCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `mendcode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth).

##### Configuración

Personalizar a través de `mendcode.json`:
...
Para utilizar su suscripción GitHub Copilot con MendCode:
...
Para utilizar Google Vertex AI con MendCode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta MendCode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de MendCode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session) para registrar automáticamente cada conversación MendCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación MendCode enumerada como una sesión separada.
...
Puede configurar MendCode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar MendCode para usar modelos locales a través de LM Studio.
...
Puedes configurar MendCode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para MendCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/MendCode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con MendCode:
...
3. Copie la clave API para usarla en MendCode.
...
6. **Importante**: Antes de usar modelos en la nube en MendCode, debe obtener la información del modelo localmente:
...
MendCode Zen es una lista de modelos probados y verificados proporcionada por el equipo MendCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>MendCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **MendCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave MendCode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración MendCode.
...
5. También puedes personalizarlos a través de tu configuración MendCode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con MendCode:
...
También puede personalizar modelos a través de su configuración MendCode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con MendCode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `mendcode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a MendCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `mend auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración MendCode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración MendCode.
