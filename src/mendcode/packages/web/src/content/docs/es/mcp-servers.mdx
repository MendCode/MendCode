...
Puede agregar herramientas externas a MendCode utilizando el _Model Context Protocol_ o MCP. MendCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [MendCode Config](/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
MendCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, MendCode:
...
Si el servidor requiere autenticación, MendCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `mend mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, MendCode almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en MendCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración MendCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar MendCode a su cuenta Sentry.
