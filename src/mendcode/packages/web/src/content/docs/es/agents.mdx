...
Hay dos tipos de agentes en MendCode; agentes primarios y subagentes.
...
MendCode viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
MendCode viene con dos subagentes integrados, **General** y **Explore**. Veremos esto a continuación.

---

## Integrados

MendCode viene con dos agentes primarios integrados y dos subagentes integrados.
...
Configure agentes en su archivo de configuración `mendcode.json`:
...
Si no se especifica ninguna temperatura, MendCode utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global MendCode como para la configuración específica del proyecto.
...
El ID del modelo en su configuración MendCode usa el formato `provider/model-id`. Por ejemplo, si usa [MendCode Zen](/docs/zen), usaría `MendCode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `mend models` para ver una lista de los modelos disponibles.
:::
