...
Las habilidades del agente permiten a MendCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
MendCode busca estas ubicaciones:

- Configuración del proyecto: `.mendcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, MendCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.mendcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.
...
Crea `.mendcode/skills/git-release/SKILL.md` así:
...
MendCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `mendcode.json`:
...
**Para agentes integrados** (en `mendcode.json`):
