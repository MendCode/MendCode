...
Para depurar problemas con MendCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\MendCode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `MendCode --log-level DEBUG`.

---

## Almacenamiento

MendCode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\MendCode`
...
MendCode Desktop ejecuta un servidor MendCode local (el sidecar `MendCode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `MendCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/opencode/mendcode.jsonc` (o `~/.config/opencode/mendcode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/mendcode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\MendCode\mendcode.jsonc`
...
MendCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\MendCode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.mendcode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que MendCode pueda reconstruirlo.

1. Salga de MendCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/MendCode`
- **Linux**: eliminar `~/.cache/MendCode` (o ejecutar `rm -rf ~/.cache/MendCode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\MendCode`

3. Reinicie el escritorio MendCode.

---

### Solucionar problemas de conexión del servidor

MendCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `mendcode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio MendCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de MendCode.

---

### Notificaciones que no se muestran

MendCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para MendCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio MendCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio MendCode):

- `MendCode.settings.dat` (URL del servidor predeterminado de escritorio)
- `MendCode.global.dat` y `MendCode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con MendCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/MendCode/issues**](/docs/troubleshooting)
...
3. Asegúrese de tener la última versión con `mend upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `mend models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/MendCode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\MendCode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. MendCode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/MendCode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\MendCode`

2. Reinicie MendCode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a MendCode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
MendCode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
