---
title: Complementos
description: Escriba sus propios complementos para extender MendCode.
---

Los complementos le permiten extender MendCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de MendCode.
...
- `.mendcode/plugins/` - Complementos a nivel de proyecto
...
```json title="mendcode.json"
{
  ...
  "plugin": ["MendCode-helicone-session", "MendCode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/MendCode/node_modules/`.
...
1. Configuración global (`~/.config/opencode/mendcode.json`)
2. Configuración del proyecto (`mendcode.json`)
3. Directorio global de complementos (`~/.config/opencode/plugins/`)
4. Directorio de complementos del proyecto (`.mendcode/plugins/`)
...
```json title=".mendcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
MendCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".mendcode/plugins/my-plugin.ts"
...
```js title=".mendcode/plugins/example.js"
...
- `client`: Un cliente SDK MendCode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar MendCode.
...
```js title=".mendcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "MendCode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio MendCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que MendCode lea archivos `.env`:

```javascript title=".mendcode/plugins/env-protection.js"
...
```javascript title=".mendcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a MendCode:

```ts title=".mendcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@mendcode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que MendCode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para MendCode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](/docs/sdk) para obtener más detalles.
...
```ts title=".mendcode/plugins/compaction.ts"
import type { Plugin } from "@mendcode/plugin"
...
```ts title=".mendcode/plugins/custom-compaction.ts"
import type { Plugin } from "@mendcode/plugin"
