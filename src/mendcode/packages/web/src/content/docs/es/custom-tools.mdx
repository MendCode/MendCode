...
description: Cree herramientas que LLM pueda llamar en MendCode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de MendCode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.mendcode/tools/` de tu proyecto.
...
```ts title=".mendcode/tools/database.ts" {1}
...
```ts title=".mendcode/tools/math.ts"
import { tool } from "@mendcode/plugin"
...
```ts title=".mendcode/tools/bash.ts"
import { tool } from "@mendcode/plugin"
...
```ts title=".mendcode/tools/project.ts" {8}
import { tool } from "@mendcode/plugin"
...
```python title=".mendcode/tools/add.py"
import sys
...
```ts title=".mendcode/tools/python-add.ts" {10}
import { tool } from "@mendcode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".mendcode/tools/add.py")
