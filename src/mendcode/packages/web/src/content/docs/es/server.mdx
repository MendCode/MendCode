---
title: Servidor
description: Interactuar con el servidor MendCode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `mend serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente MendCode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `MendCode.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `MendCode`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `mend serve` como a `mend web`.
...
Cuando ejecuta `MendCode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor MendCode para interactuar con MendCode mediante programación.
:::

Esta arquitectura permite que MendCode admita múltiples clientes y le permite interactuar con MendCode mediante programación.

Puede ejecutar `mend serve` para iniciar un servidor independiente. Si tienes el
MendCode TUI ejecutándose, `mend serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos MendCode [IDE](/docs/ide).
...
El servidor MendCode expone las siguientes API.
