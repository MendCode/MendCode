...
Las herramientas permiten que LLM realice acciones en su código base. MendCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en MendCode.
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor MendCode o cuando la variable de entorno `OPENCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar MendCode:

```bash
OPENCODE_ENABLE_EXA=1 MendCode
```
