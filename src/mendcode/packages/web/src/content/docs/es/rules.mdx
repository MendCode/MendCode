---
title: Reglas
description: Establezca instrucciones personalizadas para MendCode.
---

Puede proporcionar instrucciones personalizadas a MendCode creando un archivo `AGENTS.md`. Esto es similar a las reglas del Cursor. Contiene instrucciones que se incluirán en el contexto de LLM para personalizar su comportamiento para su proyecto específico.

---

## Inicializar

Para crear un nuevo archivo `AGENTS.md`, puede ejecutar el comando `/init` en MendCode.

:::tip
Debes enviar el archivo `AGENTS.md` de tu proyecto a Git.
:::

Esto escaneará su proyecto y todo su contenido para comprender de qué se trata y generar un archivo `AGENTS.md` con él. Esto ayuda a MendCode a navegar mejor por el proyecto.
...
MendCode también admite la lectura del archivo `AGENTS.md` desde múltiples ubicaciones. Y esto tiene diferentes propósitos.
...
También puedes tener reglas globales en un archivo `~/.config/opencode/AGENTS.md`. Esto se aplica en todas las sesiones MendCode.
...
Para los usuarios que migran desde Claude Code, MendCode admite las convenciones de archivos de Claude Code como alternativas:
...
Cuando se inicia MendCode, busca archivos de reglas en este orden:
...
Puede especificar archivos de instrucciones personalizados en su `mendcode.json` o el `~/.config/opencode/mendcode.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
Si bien MendCode no analiza automáticamente las referencias de archivos en `AGENTS.md`, puedes lograr una funcionalidad similar de dos maneras:

### Usar mendcode.json

El enfoque recomendado es utilizar el campo `instructions` en `mendcode.json`:
...
Puede enseñarle a MendCode a leer archivos externos proporcionando instrucciones explícitas en su `AGENTS.md`. He aquí un ejemplo práctico:
...
- Asegúrese de que MendCode cargue archivos solo cuando sea necesario para la tarea específica

:::tip
Para monorepos o proyectos con estándares compartidos, usar `mendcode.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
