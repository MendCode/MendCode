...
With MendCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, MendCode uses our own `MendCode` theme.
...
MendCode comes with several built-in themes.
...
- Want MendCode to match their terminal's appearance
...
MendCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
3. **Project root directory** - Defined in the `<project-root>/.mendcode/themes/*.json`
4. **Current working directory** - Defined in `./.mendcode/themes/*.json`
