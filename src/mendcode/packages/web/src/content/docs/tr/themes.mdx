...
MendCode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak MendCode kendi `MendCode` temasını kullanır.
...
MendCode birden fazla yerleşik temayla gelir.
...
- MendCode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
MendCode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
3. **Proje kök dizini** - `<project-root>/.mendcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.mendcode/themes/*.json`
