...
Ajan becerileri, MendCode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
MendCode su konumlari tarar:

- Proje konfigurasyonu: `.mendcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda MendCode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.mendcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.
...
`.mendcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
MendCode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`mendcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`mendcode.json` icinde):
