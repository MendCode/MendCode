---
title: Kurallar
description: MendCode için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak MendCode için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.

---

## Başlat

Yeni bir `AGENTS.md` dosyası oluşturmak için `/init` komutunu MendCode'da çalıştırabilirsiniz.

:::tip
Projenizin `AGENTS.md` dosyasını Git'e göndermelisiniz.
:::

Bu, projenin neyle ilgili olduğunu anlamak için projenizi ve tüm içeriğini tarayacak ve onunla bir `AGENTS.md` dosyası oluşturacaktır. Bu, MendCode'un projede daha iyi gezinmesine yardımcı olur.
...
MendCode ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/opencode/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm MendCode oturumlarına uygulanır.
...
MendCode, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:
...
MendCode başlatıldığında kural dosyalarını şu sırayla arar:
...
Özel talimat dosyalarını `mendcode.json` veya global `~/.config/opencode/mendcode.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
MendCode, `AGENTS.md`'daki dosya referanslarını otomatik olarak ayrıştırmasa da, benzer işlevleri iki şekilde elde edebilirsiniz:

### mendcode.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `mendcode.json` alanını kullanmaktır:
...
`AGENTS.md`'nizde açık talimatlar sağlayarak MendCode'u harici dosyaları okumayı öğretebilirsiniz. İşte pratik bir örnek:
...
- MendCode'un dosyaları yalnızca belirli bir görev için gerektiğinde yüklediğinden emin olun

:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `mendcode.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
