...
MendCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
MendCode Go şu anda beta olarak mevcut.
:::

Go, MendCode'daki diğer sağlayıcılar gibi çalışır. MendCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve MendCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
MendCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

MendCode Go, MendCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>MendCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `MendCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye MendCode Go'ya abone olabilir.
:::
...
MendCode Go aşağıdaki limitleri içerir:
...
MendCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `MendCode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `MendCode-go/kimi-k2.6` kullanmalısınız.
...
MendCode Go'yu şu amaçlarla oluşturduk:
...
4. MendCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
