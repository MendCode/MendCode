---
title: ACP Desteği
description: MendCode'u ACP uyumlu herhangi bir editörde kullanın.
---

MendCode, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
MendCode'u ACP aracılığıyla kullanmak için düzenleyicinizi `mend acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, MendCode'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'MendCode' ajanını kullanın.
...
MendCode'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı MendCode'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
MendCode, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- MendCode yapılandırmanızda yapılandırılmış MCP sunucuları
