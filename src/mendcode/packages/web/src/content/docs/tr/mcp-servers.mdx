...
_Model Bağlam Protokolü_ veya MCP kullanarak MendCode'a harici araçlar ekleyebilirsiniz. MendCode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [MendCode Config](/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
MendCode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde MendCode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa MendCode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `mend mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından MendCode, belirteçleri `~/.local/share/opencode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra MendCode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi MendCode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve MendCode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
