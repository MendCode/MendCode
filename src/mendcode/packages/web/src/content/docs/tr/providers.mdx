---
title: Sağlayıcılar
description: MendCode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı MendCode yapılandırmanızda yapılandırın.
...
Sağlayıcıları MendCode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
MendCode Zen, MendCode ekibi tarafından sağlanan modellerin bir listesidir.
MendCode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz MendCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, MendCode'u seçin ve [MendCode.ai/auth](https://MendCode.ai/auth)'ye gidin.
...
MendCode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## MendCode Go

MendCode Go, MendCode ile iyi çalıştığı test edilmiş ve doğrulanmış, MendCode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `MendCode Go`'yu seçin ve [MendCode console](/auth) adresine gidin.
...
Amazon Bedrock'u MendCode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   MendCode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `mendcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi MendCode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
MendCode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   MendCode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   MendCode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in mend auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
MendCode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. MendCode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`mendcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`mendcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi MendCode'la kullanmak için:
...
Google Vertex AI'yi MendCode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları MendCode çalıştırılırken ayarlayın.
...
Helicone'un MendCode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her MendCode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her MendCode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
MendCode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla MendCode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla MendCode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini MendCode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/MendCode)'a bakın.
:::
...
Ollama Cloud'u MendCode ile kullanmak için:
...
3. MendCode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: MendCode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
MendCode Zen, MendCode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>MendCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **MendCode Zen**'i arayın.

   ```txt
   /connect
   ```

3. MendCode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   MendCode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca MendCode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi MendCode ile kullanmak için:
...
Modelleri MendCode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı MendCode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `mendcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları MendCode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `mend auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için MendCode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, MendCode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
