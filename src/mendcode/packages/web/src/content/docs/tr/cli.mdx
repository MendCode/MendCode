---
title: CLI
description: MendCode CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
mend
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, MendCode ile programlı olarak etkileşim kurmanıza olanak tanır.
...
MendCode terminal kullanıcı arayüzünü başlatır.
...
MendCode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

MendCode için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir MendCode arka uç sunucusuna bir terminal ekleyin.

```bash
mend attach [url]
```

Bu, TUI öğesinin uzak bir MendCode arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `mend`) |
...
MendCode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `mend auth login` kullanabilirsiniz. Bu `~/.local/share/opencode/auth.json`'de saklanıyor.

```bash
mend auth login
```

MendCode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları MendCode'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek MendCode'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `mend serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir MendCode sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `mend`) |
...
API erişimi için headless bir MendCode sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
mend serve
```

Bu, TUI arayüzü olmadan MendCode işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `mend` şeklindedir).
...
Tüm MendCode oturumlarını listeleyin.
...
MendCode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya MendCode paylaşımından URL oturum verilerini içe aktarın.

```bash
mend import <file>
```

Yerel bir dosyadan veya MendCode paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir MendCode sunucusu başlatın.

```bash
mend web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla MendCode'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `mend` şeklindedir).
...
MendCode'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
MendCode'u en son sürüme veya belirli bir sürüme günceller.
...
MendCode CLI aşağıdaki global bayrakları destekler.
...
MendCode ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `mend`) |
