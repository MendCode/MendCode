...
MendCode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
MendCode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
MendCode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

MendCode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `mendcode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse MendCode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global MendCode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
MendCode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [MendCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `MendCode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `mend models` komutunu çalıştırın.
:::
