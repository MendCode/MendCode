...
Create `.mendcode/commands/test.md`:
...
You can add custom commands through the MendCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your MendCode [config](/docs/config):
...
MendCode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
