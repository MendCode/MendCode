...
You can add external tools to MendCode using the _Model Context Protocol_, or MCP. MendCode supports both local and remote servers.
...
You can define MCP servers in your [MendCode Config](/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
MendCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, MendCode will:
...
If the server requires authentication, MendCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `mend mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MendCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in MendCode, alongside built-in tools. So you can manage them through the MendCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect MendCode to your Sentry account.
