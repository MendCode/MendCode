---
title: ACP Support
description: Use MendCode in any ACP-compatible editor.
---

MendCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use MendCode via ACP, configure your editor to run the `mend acp` command.

The command starts MendCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'MendCode' agent in the AI Chat agent selector.
...
To use MendCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use MendCode as the ACP agent for chat.
...
MendCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your MendCode config
