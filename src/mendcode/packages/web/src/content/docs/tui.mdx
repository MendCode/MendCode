---
title: TUI
description: Using the MendCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running MendCode starts the TUI for the current directory.
...
When using the MendCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to MendCode. Allows you to select from available providers and add their API keys.
...
Exit MendCode. _Aliases_: `/quit`, `/q`
...
This is separate from `mendcode.json`, which configures server/runtime behavior.
