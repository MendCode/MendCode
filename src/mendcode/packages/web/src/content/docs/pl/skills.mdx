...
Umiejętności agenta pozwalają MendCode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
MendCode przeszukuje te lokalizacje:

- Project config: `.mendcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu MendCode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.mendcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.
...
Utwórz `.mendcode/skills/git-release/SKILL.md` w ten sposób:
...
MendCode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `mendcode.json`:
...
**Dla agentów wbudowanych** (w `mendcode.json`):
