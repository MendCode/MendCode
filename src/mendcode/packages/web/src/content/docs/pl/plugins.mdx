---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć MendCode.
---

Wtyczki umożliwiające wyłączenie MendCode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania MendCode.
...
- `.mendcode/plugins/` - Wtyczki na poziomie projektu
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/MendCode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/opencode/mendcode.json`)
2. Konfiguracja projektu (`mendcode.json`)
3. Globalny katalog wtyczek (`~/.config/opencode/plugins/`)
4. Katalog wtyczek projektu (`.mendcode/plugins/`)
...
MendCode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z MendCode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do MendCode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej MendCode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj MendCode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do MendCode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod MendCode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](/docs/sdk).
