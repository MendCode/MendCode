...
Aby debugować problemy z MendCode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\MendCode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `MendCode --log-level DEBUG`.

---

## Przechowywanie danych

MendCode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\MendCode`
...
MendCode Desktop uruchamia w tle lokalny serwer MendCode (sidecar `MendCode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `MendCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opencode/mendcode.jsonc` (lub `~/.config/opencode/mendcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/mendcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\MendCode\mendcode.jsonc`
...
MendCode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\MendCode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby MendCode mógł ją odbudować.

1. Całkowicie zamknij MendCode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/MendCode`
- **Linux**: usuń `~/.cache/MendCode` (lub uruchom `rm -rf ~/.cache/MendCode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\MendCode`

3. Uruchom ponownie MendCode Desktop.

---

### Napraw problemy z połączeniem z serwerem

MendCode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `mendcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows MendCode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami MendCode.

---

### Brak powiadomień

MendCode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla MendCode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie MendCode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji MendCode Desktop):

- `MendCode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `MendCode.global.dat` i `MendCode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z MendCode:
...
3. Upewnij się, że masz najnowszą wersję z `mend upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `mend models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\MendCode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. MendCode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\MendCode`

2. Uruchom ponownie kod MendCode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to MendCode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
MendCode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
