---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji MendCode.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim MendCode
config.
...
MendCode Zen to lista modeli dostarczonych przez zespół MendCode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z MendCode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od MendCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `MendCode Zen` i przejdź do [MendCode console](/auth).
...
Działa jak każdy inny dostawca w MendCode i jest całkowicie opcjonalny w użyciu.

---

## MendCode Go

MendCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół MendCode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z MendCode.

1. Uruchom polecenie `/connect` w TUI, wybierz `MendCode Go` i przejdź do [MendCode console](/auth).
...
Aby używać Amazon Bedrock z MendCode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania MendCode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `mendcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w MendCode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować MendCode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod MendCode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji MendCode.
...
MendCode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować MendCode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`mendcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth).

##### Konfiguracja

Customize through `mendcode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem MendCode:
...
Aby używać Google Vertex AI z MendCode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania MendCode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą MendCode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session), aby automatycznie rejestrować każdą konwersację MendCode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja MendCode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować MendCode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować MendCode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować MendCode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem MendCode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/MendCode).
:::
...
Aby korzystać z Ollama Cloud z MendCode:
...
3. Skopiuj klucz API do użycia w MendCode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w MendCode musisz pobrać informacje o modelu lokalnie:
...
MendCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół MendCode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>MendCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **MendCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API MendCode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji MendCode.
...
5. Możesz je także dostosować za pomocą konfiguracji MendCode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z MendCode:
...
Możesz także dostosować modele za pomocą konfiguracji MendCode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z MendCode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `mendcode.json` w katalogu projektu:
...
Pola `limit` pozwalają MendCode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `mend auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację MendCode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji MendCode.
