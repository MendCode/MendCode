...
Utwórz `.mendcode/commands/test.md`:
...
Możesz zdefiniować polecenia w `mendcode.json` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji MendCode:
...
MendCode zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
