---
title: Serwer
description: Interakcja z serwerem MendCode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `mend serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient MendCode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `MendCode.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `MendCode` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `mend serve`, jak i `mend web`.
...
Kiedy uruchomisz `MendCode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera MendCode do programowej interakcji z MendCode.
:::

Ta architektura umożliwia obsługę wielu klientów przez MendCode i programową interakcję z kodem otwartym.

Możesz uruchomić `mend serve`, aby uruchomić samodzielny serwer. Jeśli masz
MendCode TUI działa, `mend serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki MendCode [IDE](/docs/ide).
...
Serwer MendCode udostępnia następujące interfejsy API.
