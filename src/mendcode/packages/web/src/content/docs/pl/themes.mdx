...
Dzięki MendCode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie MendCode używa naszego własnego motywu `MendCode`.
...
MendCode ma kilka wbudowanych motywów.
...
- Chcą, aby MendCode pasował do wyglądu ich terminala
...
MendCode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.mendcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.mendcode/themes/*.json`
