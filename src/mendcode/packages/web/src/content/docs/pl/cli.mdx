---
title: CLI
description: Opcje i polecenia wiersza poleceń MendCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI MendCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
mend
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać MendCode programowo.
...
Uruchamia interfejs terminalowy (TUI) MendCode.
...
Interfejs CLI MendCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami MendCode.
...
Dołącz terminal do już działającego serwera backendu MendCode uruchomionego za pomocą `serve` lub `web`.

```bash
mend attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem MendCode. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `mend`) |
...
MendCode integruje się z wieloma dostawcami. Możesz użyć `mend auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

Podczas uruchamiania MendCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w MendCode.
...
Uruchom MendCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `mend serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera MendCode (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `mend`) |
...
Uruchom serwer MendCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
mend serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API MendCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `mend`).
...
Lista wszystkich sesji MendCode.
...
Pokaż statystyki wykorzystania tokenów i sesji MendCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania MendCode.
...
Uruchom serwer MendCode z interfejsem internetowym.

```bash
mend web
```

Uruchamia to serwer HTTP i udostępnia MendCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `mend`).
...
Odinstaluj MendCode i usuń wszystkie powiązane pliki.
...
Aktualizuj MendCode do najnowszej lub określonej wersji.
...
Interfejs CLI MendCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
MendCode można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `mend`)    |
