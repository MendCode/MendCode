...
W MendCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
MendCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
MendCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

MendCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `mendcode.json`:
...
Jeśli nie zostanie skonfigurowana, MendCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji MendCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji MendCode w `provider/model-id`. Na przykład, [MendCode Zen](/docs/zen), użyj `MendCode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `mend models`, aby zobaczyć dostępne modele.
:::
