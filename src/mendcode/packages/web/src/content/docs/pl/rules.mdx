---
title: Zasady
description: Ustaw niestandardowe instrukcje dla MendCode.
---

Możesz dostarczyć niestandardowe instrukcje do MendCode, tworząc plik `AGENTS.md`. Jest to podobne do zasad Cursora. Zawiera instrukcje, które zostaną uwzględnione w kontekście LLM, aby dostosować jego zachowanie do konkretnego projektu.
...
MendCode obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/opencode/AGENTS.md`. Jest to stosowane we wszystkich sesjach MendCode.
...
W przypadku użytkowników migrujących z Claude Code MendCode obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:
...
Po uruchomieniu MendCode szuka plików reguł w następującej kolejności:
...
Możesz określić niestandardowe pliki instrukcji w swoim `mendcode.json` lub globalnym `~/.config/opencode/mendcode.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Chociaż MendCode nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:

### Używanie mendcode.json

Zalecanym podejściem jest użycie pola `instructions` w `mendcode.json`:
...
Możesz nauczyć MendCode czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że MendCode ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania

:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `mendcode.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
