...
Możesz dodać dodatkowe informacje do MendCode za pomocą _Model Context Protocol_, w skrócie MCP. MendCode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [MendCode Config](/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
MendCode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, MendCode:
...
Jeśli serwer wymaga uwierzytelnienia, MendCode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `mend mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji MendCode bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w MendCode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie MendCode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć MendCode z kontem Sentry.
