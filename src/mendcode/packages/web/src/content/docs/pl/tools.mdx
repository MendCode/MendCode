...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. MendCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w MendCode.
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy MendCode lub gdy zmienna środowiskowa `OPENCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu MendCode:
