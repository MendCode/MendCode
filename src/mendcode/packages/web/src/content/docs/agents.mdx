...
There are two types of agents in MendCode; primary agents and subagents.
...
MendCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
MendCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

MendCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `mendcode.json` config file:
...
If no temperature is specified, MendCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
MendCode also lets you disable the built-in primary agents when you provide another primary/default agent:
...
```json title="mendcode.json"
{
  "agent": {
    "build": { "disable": true },
    "plan": { "disable": true },
    "custom_primary": { "mode": "primary" }
  },
  "default_agent": "custom_primary"
}
```

If both `build` and `plan` are disabled without another visible primary default, startup fails with an actionable config error.
...
This path is relative to where the config file is located. So this works for both the global MendCode config and the project specific config.
...
The model ID in your MendCode config uses the format `provider/model-id`. For example, if you're using [MendCode Zen](/docs/zen), you would use `MendCode/gpt-5.1-codex` for GPT 5.1 Codex.

Subagents launched through the Task tool resolve models in this order:

1. `model` passed by the Task tool call
2. `agent.<subagent>.model`
3. global `subagent_model`
4. the parent chat model
...
```json title="mendcode.json" {3-6,9-12}
{
  "$schema": "https://MendCode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="mendcode.json" {3-5,8-10}
{
  "$schema": "https://MendCode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `mend models` to see a list of the available models.
:::
