---
title: المزوّدون
description: استخدام أي موفّر LLM في MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم MendCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات MendCode.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات MendCode.
...
MendCode Zen هي قائمة نماذج يوفّرها فريق MendCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع MendCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع MendCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر MendCode، ثم انتقل إلى [MendCode.ai/auth](https://MendCode.ai/auth).
...
يعمل مثل أي مزوّد آخر في MendCode واستخدامه اختياري بالكامل.

---

## MendCode Go

MendCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق MendCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع MendCode.

1. شغّل الأمر `/connect` في TUI، واختر `MendCode Go`، ثم انتقل إلى [MendCode console](/auth).
...
لاستخدام Amazon Bedrock مع MendCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل MendCode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `mendcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل MendCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين MendCode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل MendCode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات MendCode.
...
تخزين الرمز في مساحة تخزين مصادقة MendCode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم MendCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر MendCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`mendcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth).

##### التهيئة

خصّص عبر `mendcode.json`:
...
لاستخدام اشتراك GitHub Copilot مع MendCode:
...
لاستخدام Google Vertex AI مع MendCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر MendCode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session) لتسجيل كل محادثة MendCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة MendCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط MendCode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط MendCode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط MendCode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع MendCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/MendCode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع MendCode:
...
3. انسخ مفتاح API لاستخدامه في MendCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في MendCode، يجب سحب معلومات النموذج محليا:
...
MendCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق MendCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>MendCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **MendCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ MendCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات MendCode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات MendCode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع MendCode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات MendCode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع MendCode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `mendcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ MendCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `mend auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات MendCode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات MendCode.
