---
title: الخادم
description: تفاعل مع خادم MendCode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `mend serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل MendCode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `MendCode.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `MendCode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `mend serve` و `mend web`.
...
عند تشغيل `MendCode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم MendCode للتفاعل مع MendCode برمجيا.
:::

تتيح هذه البنية لـ MendCode دعم عدة عملاء وتمكّنك من التفاعل مع MendCode برمجيا.

يمكنك تشغيل `mend serve` لبدء خادم مستقل. إذا كانت واجهة MendCode في terminal (TUI)
قيد التشغيل، فسيبدأ `mend serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات MendCode لـ [IDE](/docs/ide).
...
يعرض خادم MendCode واجهات API التالية.
