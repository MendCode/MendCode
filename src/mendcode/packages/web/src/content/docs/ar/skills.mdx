...
تتيح مهارات الوكيل لـ MendCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث MendCode في هذه المواقع:

- إعدادات المشروع: `.mendcode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج MendCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.mendcode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.
...
أنشئ `.mendcode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض MendCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `mendcode.json`:
...
**للوكلاء المدمجين** (في `mendcode.json`):
