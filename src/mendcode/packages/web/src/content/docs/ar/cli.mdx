---
title: CLI
description: أوامر وخيارات CLI في MendCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ MendCode افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
mend
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع MendCode برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ MendCode.
...
يتضمن CLI الخاص بـ MendCode أيضا الأوامر التالية.

---

### agent

إدارة وكلاء MendCode.
...
إرفاق terminal بخادم الواجهة الخلفية لـ MendCode قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
mend attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ MendCode تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `mend`) |
...
يعتمد MendCode على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `mend auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

عند تشغيل MendCode يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في MendCode.
...
تشغيل MendCode في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `mend serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم MendCode قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `mend`) |
...
بدء خادم MendCode بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
mend serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف MendCode دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `mend`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات MendCode لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة MendCode.

```bash
mend import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة MendCode.
...
بدء خادم MendCode بدون واجهة مع واجهة ويب.

```bash
mend web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى MendCode عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `mend`).
...
إلغاء تثبيت MendCode وإزالة جميع الملفات ذات الصلة.
...
تحديث MendCode إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر MendCode الخيارات العامة التالية.
...
يمكن تهيئة MendCode باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `mend`) |
