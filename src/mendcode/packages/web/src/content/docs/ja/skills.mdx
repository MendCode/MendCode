...
エージェントスキルにより、MendCode はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
MendCode は次の場所を検索します。

- プロジェクト設定: `.mendcode/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、MendCode は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.mendcode/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。
...
次のように `.mendcode/skills/git-release/SKILL.md` を作成します。
...
MendCode では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`mendcode.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
