...
MendCode Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

:::note
MendCode Goは現在ベータ版です。
:::

GoはMendCodeの他のプロバイダーと同様に機能します。MendCode GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、MendCodeを使用するために必須ではありません。
...
:::tip
MendCodeでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
MendCode Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

MendCode Goは、MendCodeの他のプロバイダーと同様に機能します。

1. **<a href={console}>MendCode Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`MendCode Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
MendCode Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
MendCode設定の[model id](/docs/config/#models)は、`MendCode-go/<model-id>`という形式を使用します。たとえば、Kimi K2.6の場合は、設定で`MendCode-go/kimi-k2.6`を使用します。
...
4. MendCodeで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
