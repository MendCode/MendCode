...
To debug issues with MendCode, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\MendCode\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `MendCode --log-level DEBUG`.

---

## Storage

MendCode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\MendCode`
...
MendCode Desktop runs a local MendCode server (the `MendCode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `MendCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/opencode/mendcode.jsonc` (or `~/.config/opencode/mendcode.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/mendcode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\MendCode\mendcode.jsonc`
...
MendCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\MendCode\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so MendCode can rebuild it.

1. Quit MendCode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/MendCode`
- **Linux**: delete `~/.cache/MendCode` (or run `rm -rf ~/.cache/MendCode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\MendCode`

3. Restart MendCode Desktop.

---

### Fix server connection issues

MendCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `mendcode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, MendCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with MendCode's features.

---

### Notifications not showing

MendCode Desktop only shows system notifications when:

- notifications are enabled for MendCode in your OS settings, and
...
1. Quit MendCode Desktop.
2. Find and delete these files (they live in the MendCode Desktop app data directory):

- `MendCode.settings.dat` (desktop default server URL)
- `MendCode.global.dat` and `MendCode.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with MendCode:
...
### MendCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `mend upgrade`
...
To figure out what models you have access to, run `mend models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\MendCode`
...
If you encounter API call errors, this may be due to outdated provider packages. MendCode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\MendCode`

2. Restart MendCode to reinstall the latest provider packages

This will force MendCode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
MendCode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
