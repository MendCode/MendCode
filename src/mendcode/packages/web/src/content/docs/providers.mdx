---
title: Providers
description: Using any LLM provider in MendCode.
---

import config from "../../../config.mjs"
export const console = config.console

MendCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your MendCode config.
...
in the local MendCode auth store used by this installation.

---

### Config

You can customize the providers through the `provider` section in your MendCode
config.
...
MendCode Zen is a list of models provided by the MendCode team that have been
tested and verified to work well with MendCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with MendCode Zen.
:::

1. Run the `/connect` command in the TUI, select `MendCode Zen`, and head to [MendCode console](/auth).
...
It works like any other provider in MendCode and is completely optional to use.

---

## MendCode Go

MendCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the MendCode team that have been
tested and verified to work well with MendCode.

1. Run the `/connect` command in the TUI, select `MendCode Go`, and head to [MendCode console](/auth).
...
:::note
Don't see a provider here? Check the current MendCode docs and release notes for updates.
:::
...
To use Amazon Bedrock with MendCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running MendCode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY mend

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile mend

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX mend
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `mendcode.json`:
...
MendCode. Anthropic explicitly prohibits this.

Previous versions of MendCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in MendCode with zero setup:
...
You can configure MendCode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for MendCode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX mend run
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX mend run
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your MendCode config.
...
GitLab Duo support in MendCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

MendCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store the token in MendCode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
MendCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock MendCode
to only use your own GitLab-hosted instance, add the following to your
`mendcode.json` file. It is also recommended to disable session sharing.
...
More documentation is available on the GitLab auth compatibility package homepage.
...
When a `duo-workflow-*` model is selected, MendCode will:
...
4. Route tool execution requests through MendCode's permission-gated tool system
...
MendCode's public GitLab tools plugin package is not published yet.

---

### GitHub Copilot

To use your GitHub Copilot subscription with MendCode:
...
To use Google Vertex AI with MendCode:
...
2. Set the required environment variables:
   ...
   Set them while running MendCode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id mend
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through MendCode, you can always configure it yourself.
...
```jsonc title="~/.config/opencode/mendcode.jsonc (compatibility path)"
{
  "$schema": "https://MendCode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. A MendCode-specific Helicone session plugin is not published yet, so session tracking currently requires your own integration.
...
You can configure MendCode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure MendCode to use local models through LM Studio.
...
You can configure MendCode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for MendCode. MendCode-specific Ollama integration docs are not published yet.
:::
...
To use Ollama Cloud with MendCode:
...
3. Copy the API key for use in MendCode.
...
6. **Important**: Before using cloud models in MendCode, you must pull the model information locally:
...
MendCode Zen is a list of tested and verified models provided by the MendCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>MendCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **MendCode Zen**.

   ```txt
   /connect
   ```

3. Enter your MendCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your MendCode config.
...
5. You can also customize them through your MendCode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' mend run
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group mend run
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with MendCode:
...
You can also customize models through your MendCode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with MendCode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `mendcode.json` file in your project directory:
...
The `limit` fields allow MendCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `mend auth list` to see if the credentials
...
2. For custom providers, check the MendCode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your MendCode config.
