---
title: Server
description: Interact with MendCode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `mend serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an MendCode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `MendCode.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `MendCode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `mend serve` and `mend web`.
...
When you run `MendCode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the MendCode server to interact with MendCode programmatically.
:::

This architecture lets MendCode support multiple clients and allows you to interact with MendCode programmatically.

You can run `mend serve` to start a standalone server. If you have the
MendCode TUI running, `mend serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the MendCode [IDE](/docs/ide) plugins.
...
The MendCode server exposes the following APIs.
