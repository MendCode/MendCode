---
title: Support ACP
description: Utilisez MendCode dans n’importe quel éditeur compatible ACP.
---

MendCode prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser MendCode via ACP, configurez votre éditeur pour exécuter la commande `mend acp`.

La commande démarre MendCode en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "MendCode": {
      "command": "/absolute/path/bin/mend",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « MendCode » dans le sélecteur d'agent AI Chat.
...
Pour utiliser MendCode comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser MendCode comme agent ACP pour le chat.
...
MendCode fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration MendCode
