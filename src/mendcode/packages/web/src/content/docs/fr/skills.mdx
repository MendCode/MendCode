...
Les compétences d'agent permettent à MendCode de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
MendCode recherche ces emplacements :

- Configuration du projet : `.mendcode/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, MendCode remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.mendcode/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.
...
Créez `.mendcode/skills/git-release/SKILL.md` comme ceci :
...
MendCode répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `mendcode.json` :
...
**Pour les agents intégrés** (dans `mendcode.json`) :
