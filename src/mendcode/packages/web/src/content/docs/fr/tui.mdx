---
title: TUI
description: Utilisation de l’interface utilisateur du terminal MendCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de MendCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le MendCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à MendCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `mendcode.json`, qui configure le comportement du serveur/d'exécution.
