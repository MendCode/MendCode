...
Il existe deux types d'agents dans MendCode ; agents primaires et sous-agents.
...
MendCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
MendCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

MendCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `mendcode.json` :
...
Si aucune température n'est spécifiée, MendCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale MendCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration MendCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [MendCode Zen](/docs/zen), vous utiliserez `MendCode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `mend models` pour voir une liste des modèles disponibles.
:::
