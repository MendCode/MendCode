---
title: Serveur
description: Interagissez avec le serveur MendCode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `mend serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client MendCode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `MendCode.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `MendCode`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `mend serve` et à `mend web`.
...
Lorsque vous exécutez `MendCode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur MendCode pour interagir avec MendCode par programmation.
:::

Cette architecture permet à MendCode de prendre en charge plusieurs clients et vous permet d'interagir avec MendCode par programme.

Vous pouvez exécuter `mend serve` pour démarrer un serveur autonome. Si vous avez le
MendCode TUI en cours d'exécution, `mend serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins MendCode [IDE](/docs/ide).
...
Le serveur MendCode expose les API suivantes.
