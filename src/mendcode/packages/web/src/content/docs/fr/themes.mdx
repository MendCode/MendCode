...
Avec MendCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, MendCode utilise notre propre thème `MendCode`.
...
MendCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que MendCode corresponde à l'apparence de leur terminal
...
MendCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
3. **Répertoire racine du projet** - Défini dans `<project-root>/.mendcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.mendcode/themes/*.json`
...
```bash no-frame
mkdir -p .mendcode/themes
vim .mendcode/themes/my-theme.json
```
