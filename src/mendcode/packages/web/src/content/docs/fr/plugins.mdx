---
title: Plugins
description: Écrivez vos propres plugins pour étendre MendCode.
---

Les plugins vous permettent d'étendre MendCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de MendCode.
...
- `.mendcode/plugins/` - Plugins au niveau du projet
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/MendCode/node_modules/`.
...
1. Configuration globale (`~/.config/opencode/mendcode.json`)
2. Configuration du projet (`mendcode.json`)
3. Répertoire global des plugins (`~/.config/opencode/plugins/`)
4. Répertoire des plugins du projet (`.mendcode/plugins/`)
...
MendCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK MendCode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre MendCode.
...
:::note
Si vous utilisez l'application de bureau MendCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez MendCode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à MendCode :
...
L'assistant `tool` crée un outil personnalisé que MendCode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour MendCode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](/docs/sdk) pour plus de détails.
