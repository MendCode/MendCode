...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez MendCode.
...
Voici plusieurs modèles qui fonctionnent bien avec MendCode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration MendCode.

```json title="mendcode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [MendCode Zen](/docs/zen), vous utiliserez `MendCode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="mendcode.jsonc" {7-12,19-24}
{
  "$schema": "https://MendCode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="mendcode.jsonc" {6-21}
{
  "$schema": "https://MendCode.ai/config.json",
  "provider": {
    "MendCode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. MendCode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

MendCode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="mendcode.jsonc" {7-18}
{
  "$schema": "https://MendCode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque MendCode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration MendCode.
