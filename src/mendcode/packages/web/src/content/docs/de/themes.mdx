...
Mit MendCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet MendCode das `MendCode`-Theme.
...
MendCode liefert mehrere eingebaute Themes mit.
...
- MendCode optisch ans Terminal anpassen wollen
...
MendCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.mendcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.mendcode/themes/*.json`
