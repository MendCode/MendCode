---
title: Server
description: Nutze den MendCode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `mend serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein MendCode-Client nutzen kann.
...
Der Benutzername ist standardmaessig `MendCode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `mend serve` und `mend web`.
...
Wenn du `MendCode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den MendCode-Server, um MendCode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `mend serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `mend serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die MendCode-[IDE](/docs/ide)-Plugins.
...
Der MendCode-Server stellt folgende APIs bereit.
