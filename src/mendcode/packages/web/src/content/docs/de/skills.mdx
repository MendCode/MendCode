...
Agent Skills erlauben MendCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
MendCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft MendCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.mendcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.
...
Erstelle `.mendcode/skills/git-release/SKILL.md` zum Beispiel so:
...
MendCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `mendcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `mendcode.json`):
