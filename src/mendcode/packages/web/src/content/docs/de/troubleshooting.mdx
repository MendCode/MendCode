...
Wenn MendCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\MendCode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `MendCode --log-level DEBUG`.

---

## Speicher

MendCode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\MendCode`
...
MendCode Desktop startet im Hintergrund einen lokalen MendCode-Server (`MendCode-cli`-Sidecar).
...
- Nur macOS: `MendCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opencode/mendcode.jsonc` (oder `~/.config/opencode/mendcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/mendcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\MendCode\mendcode.jsonc`
...
MendCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\MendCode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit MendCode ihn neu aufbauen kann.

1. Beende MendCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/MendCode`
- **Linux**: loesche `~/.cache/MendCode` (oder fuehre aus: `rm -rf ~/.cache/MendCode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\MendCode`

3. Starte MendCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

MendCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `mendcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt MendCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit MendCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

MendCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer MendCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende MendCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von MendCode Desktop):

- `MendCode.settings.dat` (desktop default server URL)
- `MendCode.global.dat` und `MendCode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit MendCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `mend models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\MendCode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. MendCode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\MendCode`

2. Starte MendCode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt MendCode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
MendCode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
