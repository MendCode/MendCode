...
MendCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
MendCode Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in MendCode. Du abonnierst MendCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
MendCode zu verwenden.
...
MendCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

MendCode Go funktioniert wie jeder andere Provider in MendCode.

1. Du meldest dich bei **<a href={console}>MendCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `MendCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann MendCode Go abonnieren.
:::
...
MendCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner MendCode Config verwendet das Format `MendCode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `MendCode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben MendCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit MendCode zu nutzen.
