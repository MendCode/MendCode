---
title: CLI
description: MendCode CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der MendCode CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
mend
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit MendCode interagieren.
...
Starten Sie die Terminalbenutzeroberfläche MendCode.
...
Der MendCode CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für MendCode verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden MendCode-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
mend attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend MendCode. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `mend`) |
...
MendCode wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `mend auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/opencode/auth.json` gespeichert.

```bash
mend auth login
```

Wenn MendCode startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in MendCode sehen möchten.
...
Führen Sie MendCode im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `mend serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden MendCode-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `mend`)                  |
...
Starten Sie einen Headless-MendCode-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
mend serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf MendCode-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `mend`).
...
Listen Sie alle MendCode-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre MendCode-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer MendCode-Freigabe-URL.

```bash
mend import <file>
```

Sie können aus einer lokalen Datei oder einer MendCode-Freigabe-URL importieren.
...
Starten Sie einen Headless-MendCode-Server mit einer Weboberfläche.

```bash
mend web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf MendCode zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `mend`).
...
Deinstallieren Sie MendCode und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert MendCode auf die neueste Version oder eine bestimmte Version.
...
Der MendCode CLI akzeptiert die folgenden globalen Flags.
...
MendCode kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `mend`) |
