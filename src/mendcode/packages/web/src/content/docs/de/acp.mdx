...
description: Verwenden Sie MendCode in jedem ACP-kompatiblen Editor.
---

MendCode unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um MendCode über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `mend acp`.

Der Befehl startet MendCode als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „MendCode“ in der AI Chat Agent Selector.
...
Um MendCode als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass MendCode als ACP-Agent für den Chat verwendet wird.
...
MendCode funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer MendCode-Konfiguration konfiguriert sind
