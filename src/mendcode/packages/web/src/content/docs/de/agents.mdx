...
Es gibt zwei Arten von Agenten in MendCode; Primäragenten und Subagenten.
...
MendCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `mendcode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet MendCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale MendCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer MendCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [MendCode Zen](/docs/zen) verwenden, würden Sie `MendCode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `mend models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
