---
title: Regeln
description: Lege eigene Anweisungen fuer MendCode fest.
---

Du kannst MendCode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in MendCode den Befehl `/init` aus.
...
So kann MendCode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht MendCode sie zu erweitern.
...
MendCode kann `AGENTS.md` aus mehreren Orten lesen.
...
Diese gelten in allen MendCode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt MendCode dessen Dateikonventionen als Fallback:
...
Beim Start sucht MendCode in dieser Reihenfolge nach Regeldateien:
...
Du kannst zusaetzliche Anweisungsdateien in deiner `mendcode.json` oder in `~/.config/opencode/mendcode.json` angeben.
...
MendCode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit mendcode.json

Empfohlen ist das `instructions`-Feld in der `mendcode.json`:
...
Du kannst MendCode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `mendcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
