---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in MendCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MendCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer MendCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem MendCode anpassen
config.
...
MendCode Zen ist eine Liste von Modellen, die vom MendCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit MendCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit MendCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie MendCode aus und gehen Sie zu [MendCode.ai/auth](https://MendCode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in MendCode und ist völlig optional.

---

## MendCode Go

MendCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom MendCode-Team getestet und verifiziert wurden, dass sie gut mit MendCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `MendCode Go` und gehen Sie zu [MendCode console](/auth).
...
So verwenden Sie Amazon Bedrock mit MendCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie MendCode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `mendcode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in MendCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können MendCode so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit MendCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre MendCode-Konfiguration hinzufügen.
...
um das Token im MendCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
MendCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von MendCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`mendcode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [MendCode-gitlab-auth](https://www.npmjs.com/package/MendCode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `mendcode.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit MendCode:
...
So verwenden Sie Google Vertex AI mit MendCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie MendCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über MendCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [MendCode-helicone-session](https://github.com/H2Shami/MendCode-helicone-session)-Plugin, um jede MendCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede MendCode-Konversation als separate Sitzung aufgeführt.
...
Sie können MendCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können MendCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können MendCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für MendCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/MendCode).
:::
...
So verwenden Sie Ollama Cloud mit MendCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in MendCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in MendCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
MendCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom MendCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>MendCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **MendCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren MendCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre MendCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre MendCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit MendCode:
...
Sie können Modelle auch über Ihre MendCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit MendCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `mendcode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann MendCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `mend auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die MendCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer MendCode-Konfiguration übereinstimmt.
