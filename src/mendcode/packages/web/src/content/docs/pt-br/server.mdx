---
title: Servidor
description: Interaja com o servidor MendCode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `mend serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente MendCode pode usar.
...
```bash
mend serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `MendCode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
mend serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `MendCode`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `mend serve` quanto ao `mend web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password mend serve
```
...
Quando você executa `MendCode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor MendCode para interagir com o MendCode programaticamente.
:::

Essa arquitetura permite que o MendCode suporte múltiplos clientes e permite que você interaja com o MendCode programaticamente.

Você pode executar `mend serve` para iniciar um servidor autônomo. Se você tiver o TUI do MendCode em execução, `mend serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do MendCode [IDE](/docs/ide).
...
O servidor MendCode expõe as seguintes APIs.
