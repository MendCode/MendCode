...
As habilidades do agente permitem que o MendCode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O MendCode pesquisa nesses locais:

- Configuração do projeto: `.mendcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o MendCode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.mendcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.
...
Crie `.mendcode/skills/git-release/SKILL.md` assim:
...
O MendCode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `mendcode.json`:
...
**Para agentes embutidos** (em `mendcode.json`):
