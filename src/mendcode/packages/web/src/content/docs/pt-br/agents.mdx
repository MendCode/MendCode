...
Existem dois tipos de agentes no MendCode; agentes primários e subagentes.
...
MendCode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
MendCode vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

MendCode vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `mendcode.json`:
...
Se nenhuma temperatura for especificada, o MendCode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do MendCode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do MendCode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [MendCode Zen](/docs/zen), você usaria `MendCode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `mend models` para ver uma lista dos modelos disponíveis.
:::
