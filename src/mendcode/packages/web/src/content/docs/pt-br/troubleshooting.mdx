...
Para depurar problemas com o MendCode, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\MendCode\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `MendCode --log-level DEBUG`.

---

## Armazenamento

O MendCode armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\MendCode`
...
O MendCode Desktop executa um servidor MendCode local (o sidecar `MendCode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `MendCode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/opencode/mendcode.jsonc` (ou `~/.config/opencode/mendcode.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/mendcode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\MendCode\mendcode.jsonc`
...
O MendCode também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\MendCode\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o MendCode possa reconstruí-lo.

1. Saia completamente do MendCode Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/MendCode`
- **Linux**: exclua `~/.cache/MendCode` (ou execute `rm -rf ~/.cache/MendCode`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\MendCode`

3. Reinicie o MendCode Desktop.

---

### Corrigindo problemas de conexão com o servidor

O MendCode Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `mendcode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o MendCode Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do MendCode.

---

### Notificações não aparecendo

O MendCode Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o MendCode nas configurações do seu sistema operacional, e
...
1. Saia do MendCode Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo MendCode Desktop):

- `MendCode.settings.dat` (URL do servidor padrão do desktop)
- `MendCode.global.dat` e `MendCode.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o MendCode:
...
Para descobrir quais modelos você tem acesso, execute `mend models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\MendCode`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O MendCode instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\MendCode`

2. Reinicie o MendCode para reinstalar os pacotes de provedores mais recentes

Isso forçará o MendCode a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O MendCode detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
