---
title: Plugins
description: Escreva seus próprios plugins para estender o MendCode.
---

Plugins permitem que você estenda o MendCode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do MendCode.
...
- `.mendcode/plugins/` - Plugins em nível de projeto
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/MendCode/node_modules/`.
...
1. Configuração global (`~/.config/opencode/mendcode.json`)
2. Configuração do projeto (`mendcode.json`)
3. Diretório de plugins global (`~/.config/opencode/plugins/`)
4. Diretório de plugins do projeto (`.mendcode/plugins/`)
...
O MendCode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do MendCode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o MendCode.
...
:::note
Se você estiver usando o aplicativo desktop MendCode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o MendCode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao MendCode:
...
O helper `tool` cria uma ferramenta personalizada que o MendCode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o MendCode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](/docs/sdk) para detalhes.
