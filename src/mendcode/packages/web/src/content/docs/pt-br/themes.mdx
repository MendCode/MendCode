...
Com o MendCode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o MendCode usa nosso próprio tema `MendCode`.
...
O MendCode vem com vários temas embutidos.
...
- Querem que o MendCode corresponda à aparência do seu terminal
...
O MendCode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
3. **Diretório raiz do projeto** - Definido em `<project-root>/.mendcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.mendcode/themes/*.json`
