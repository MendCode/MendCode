---
title: CLI
description: Opções e comandos da CLI do MendCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do MendCode, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
mend
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o MendCode programaticamente.
...
Inicie a interface de usuário do terminal do MendCode.
...
A CLI do MendCode também possui os seguintes comandos.

---

### agent

Gerencie agentes para o MendCode.
...
Anexe um terminal a um servidor backend do MendCode já em execução, iniciado via comandos `serve` ou `web`.

```bash
mend attach [url]
```

Isso permite usar o TUI com um backend MendCode remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `mend`) |
...
O MendCode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `mend auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

Quando o MendCode é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no MendCode.
...
Execute o MendCode em modo não interativo passando um prompt diretamente.
...
```bash "mend run"
mend run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `mend serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
mend run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor MendCode em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `mend`) |
...
Inicie um servidor MendCode sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
mend serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do MendCode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `mend`).
...
Liste todas as sessões do MendCode.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do MendCode.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do MendCode.

```bash
mend import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do MendCode.
...
Inicie um servidor MendCode sem cabeça com uma interface web.

```bash
mend web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o MendCode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `mend`).
...
Desinstale o MendCode e remova todos os arquivos relacionados.
...
Atualiza o MendCode para a versão mais recente ou uma versão específica.
...
A CLI do MendCode aceita as seguintes flags globais.
...
O MendCode pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `mend`) |
