---
title: Configuração
description: Usando a configuração JSON do MendCode.
---

Você pode configurar o MendCode usando um arquivo de configuração JSON.

---

## Formato

O MendCode suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
2. **Configuração global** (`~/.config/opencode/mendcode.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`mendcode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.mendcode`** - agentes, comandos, plugins
...
:::note
Os diretórios `.mendcode` e `~/.config/MendCode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do MendCode em `~/.config/opencode/mendcode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `mendcode.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o MendCode é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.mendcode`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.mendcode`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`MendCode.ai/config.json`**](https://MendCode.ai/config.json).

A configuração do TUI usa [**`MendCode.ai/tui.json`**](https://MendCode.ai/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `mendcode.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `mend serve` e `mend web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor MendCode.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `mendcode.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do MendCode através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o MendCode tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.mendcode/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o MendCode recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`mend run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.mendcode/commands/`. [Saiba mais aqui](/docs/commands).
...
O MendCode fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o MendCode **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o MendCode com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.mendcode/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o MendCode para usar apenas provedores específicos, em vez de desabilitá-los um a um.
