...
Le skill degli agenti permettono a MendCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
MendCode cerca in queste posizioni:

- Config di progetto: `.mendcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, MendCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.mendcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.
...
Crea `.mendcode/skills/git-release/SKILL.md` cosi':
...
MendCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `mendcode.json`:
...
**Per agenti integrati** (in `mendcode.json`):
