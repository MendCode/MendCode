---
title: CLI
description: Opzioni e comandi della CLI di MendCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di MendCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
mend
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con MendCode in modo programmatico.
...
Avvia l'interfaccia testuale di MendCode (interfaccia utente da terminale).
...
La CLI di MendCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per MendCode.
...
Collega un terminale a un backend server di MendCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
mend attach [url]
```

Questo consente di usare la TUI con un backend MendCode remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `mend`) |
...
MendCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `mend auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
mend auth login
```

Quando MendCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in MendCode.
...
Esegui MendCode in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `mend serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server MendCode in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `mend`) |
...
Avvia un server MendCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
mend serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di MendCode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `mend`).
...
Elenca tutte le sessioni MendCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni MendCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione MendCode.

```bash
mend import <file>
```

Puoi importare da un file locale o da un URL di condivisione MendCode.
...
Avvia un server MendCode headless con interfaccia web.

```bash
mend web
```

Avvia un server HTTP e apre un browser per accedere a MendCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `mend`).
...
Disinstalla MendCode e rimuove tutti i file correlati.
...
Aggiorna MendCode all'ultima versione o a una versione specifica.
...
La CLI di MendCode accetta i seguenti flag globali.
...
MendCode può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `mend`)     |
