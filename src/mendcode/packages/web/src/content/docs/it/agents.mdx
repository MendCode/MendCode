...
In MendCode esistono due tipi di agenti: agenti primari e subagenti.
...
MendCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
MendCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

MendCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `mendcode.json`:
...
Se non specifichi una temperature, MendCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di MendCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione MendCode usa il formato `provider/model-id`. Per esempio, se stai usando [MendCode Zen](/docs/zen), useresti `MendCode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `mend models` per vedere la lista dei modelli disponibili.
:::
