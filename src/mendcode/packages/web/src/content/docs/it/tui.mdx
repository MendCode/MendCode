---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di MendCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MendCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire MendCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di MendCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a MendCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da MendCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `mendcode.json`, che configura il comportamento di server/runtime.
