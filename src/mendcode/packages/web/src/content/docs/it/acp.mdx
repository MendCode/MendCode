---
title: Supporto ACP
description: Usa MendCode in qualunque editor compatibile con ACP.
---

MendCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare MendCode via ACP, configura l'editor in modo che esegua il comando `mend acp`.

Il comando avvia MendCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'MendCode' nel selettore agenti di AI Chat.
...
Per usare MendCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare MendCode come agente ACP per la chat.
...
MendCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione MendCode
