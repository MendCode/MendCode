...
description: Usa la configurazione JSON di MendCode.
---

Puoi configurare MendCode usando un file di configurazione JSON.

---

## Formato

MendCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/opencode/mendcode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`mendcode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.mendcode`** - agenti, comandi, plugin
...
:::note
Le directory `.mendcode` e `~/.config/MendCode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di MendCode in `~/.config/opencode/mendcode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `mendcode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando MendCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.mendcode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.mendcode`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`MendCode.ai/config.json`**](https://MendCode.ai/config.json).

La configurazione TUI usa [**`MendCode.ai/tui.json`**](https://MendCode.ai/tui.json).
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `mendcode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `mend serve` e `mend web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server MendCode.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `mendcode.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in MendCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, MendCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.mendcode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, MendCode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`mend run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.mendcode/commands/`. [Scopri di piu](/docs/commands).
...
MendCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, MendCode **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono MendCode con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.mendcode/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare MendCode a usare solo alcuni provider invece di disabilitarli uno per uno.
