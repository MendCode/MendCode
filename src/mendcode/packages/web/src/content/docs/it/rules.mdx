---
title: Regole
description: Imposta istruzioni personalizzate per MendCode.
---

Puoi fornire istruzioni personalizzate a MendCode creando un file `AGENTS.md`. E' simile alle regole di Cursor. Contiene istruzioni che verranno incluse nel contesto dell'LLM per personalizzarne il comportamento per il tuo progetto.

---

## Inizializza

Per creare un nuovo file `AGENTS.md`, puoi eseguire il comando `/init` in MendCode.

:::tip
Dovresti committare il file `AGENTS.md` del progetto in Git.
:::

Questo scansiona il progetto e tutti i suoi contenuti per capire di cosa si tratta e generare un file `AGENTS.md`. Questo aiuta MendCode a orientarsi meglio nel progetto.
...
MendCode supporta anche la lettura del file `AGENTS.md` da piu' posizioni, e questo serve a scopi diversi.
...
Puoi anche avere regole globali in un file `~/.config/opencode/AGENTS.md`. Queste si applicano a tutte le sessioni di MendCode.
...
Per gli utenti che migrano da Claude Code, MendCode supporta come fallback le convenzioni di file di Claude Code:
...
Quando MendCode si avvia, cerca i file di regole in questo ordine:
...
Puoi specificare file di istruzioni personalizzati nel tuo `mendcode.json` o nel globale `~/.config/opencode/mendcode.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
Anche se MendCode non interpreta automaticamente i riferimenti a file in `AGENTS.md`, puoi ottenere una funzionalita' simile in due modi:

### Usa mendcode.json

L'approccio consigliato e' usare il campo `instructions` in `mendcode.json`:
...
Puoi insegnare a MendCode a leggere file esterni fornendo istruzioni esplicite nel tuo `AGENTS.md`. Ecco un esempio pratico:
...
- Assicurati che MendCode carichi file solo quando servono per il task specifico

:::tip
Per monorepo o progetti con standard condivisi, usare `mendcode.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
