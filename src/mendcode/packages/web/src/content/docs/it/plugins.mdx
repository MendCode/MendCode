---
title: Plugin
description: Scrivi plugin per estendere MendCode.
---

I plugin ti permettono di estendere MendCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di MendCode.
...
- `.mendcode/plugins/` - plugin a livello progetto
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/MendCode/node_modules/`.
...
1. Config globale (`~/.config/opencode/mendcode.json`)
2. Config di progetto (`mendcode.json`)
3. Directory plugin globale (`~/.config/opencode/plugins/`)
4. Directory plugin di progetto (`.mendcode/plugins/`)
...
MendCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di MendCode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere MendCode.
...
:::note
Se usi l'app desktop di MendCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a MendCode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a MendCode:
...
L'helper `tool` crea uno strumento personalizzato che MendCode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in MendCode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](/docs/sdk) per i dettagli.
