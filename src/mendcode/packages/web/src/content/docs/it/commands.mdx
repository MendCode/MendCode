...
Crea `.mendcode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di MendCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di MendCode:
...
- Per progetto: `.mendcode/commands/`
...
MendCode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
