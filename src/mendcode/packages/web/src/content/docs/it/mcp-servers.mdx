...
Puoi aggiungere strumenti esterni a MendCode usando il _Model Context Protocol_, o MCP. MendCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [MendCode Config](/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
MendCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, MendCode:
...
Se il server richiede autenticazione, MendCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `mend mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, MendCode memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in MendCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione MendCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare MendCode al tuo account Sentry.
