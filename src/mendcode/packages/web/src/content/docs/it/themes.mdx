...
Con MendCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, MendCode usa il tema `MendCode`.
...
MendCode include diversi temi integrati.
...
- Vuole che MendCode corrisponda all'aspetto del terminale
...
MendCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
3. **Directory root del progetto** - in `<project-root>/.mendcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.mendcode/themes/*.json`
