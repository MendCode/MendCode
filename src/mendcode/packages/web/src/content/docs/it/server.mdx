---
title: Server
description: Interagisci con il server MendCode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `mend serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client MendCode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `MendCode.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `MendCode`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `mend serve` sia per `mend web`.
...
Quando esegui `MendCode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server MendCode per interagire con MendCode in modo programmatico.
:::

Questa architettura permette a MendCode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `mend serve` per avviare un server standalone. Se la TUI di MendCode e gia in esecuzione, `mend serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di MendCode.
...
Il server MendCode espone le seguenti API.
